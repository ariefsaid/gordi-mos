...
- **Amendment (2026-07-10, owner; ADR-0025 D26 / OD-REDESIGN-40):** an ad-hoc Task is valid without a
  Project/Process. `program_process_id` remains nullable by design. “Direct and permanent” now means that
  when a Task is classified, its cascade link goes directly to Project/Process and never routes through a
  deferred Output; it does **not** mean every Task must have a parent. Process-Run-generated Tasks retain
  their required generating Process/Run relation.
- **Current naming/model amendment:** ADR-0015 and the E7 redesign use the **Project/Process pair** with
  no umbrella term; `Initiative` and in-app `Program` below are historical. Process is a versioned
  definition, Process Run is its occurrence, and Tasks use PIC + Supervisor. The implementation-shape
  discussion below remains E6 history pending the clean baseline.
...
3. **Topology rule — additive, never inserted-between.** A Task may attach **directly** to an Initiative
   (its optional cascade parent); ad-hoc Tasks may remain unparented. When Output is built later it is an **optional side-grouping that
   also belongs to the Initiative** — never a mandatory link between Task and Initiative. So Output ships
   as `CREATE TABLE output` + a nullable `task.output_id`, with **zero re-pointing** of an existing
   Task→Initiative link. This single decision is what makes the "no rebuild later" guarantee hold.
...
- **Superseded factual-feed implementation:** the historical Daily Log conclusion established the
  useful fact-vs-work boundary, but Daily Log itself is now retired. **Signal** is the factual object;
  Process/Process Run and Tasks remain owned work (ADR-0025 D20/D25/D30).
