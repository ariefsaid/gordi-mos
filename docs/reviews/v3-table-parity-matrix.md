...
| Selection / bulk capability honesty | Selection is explicitly false at `tasks/task-collection-adapter.tsx:617-625`; no selection column in `tasks/tasks-table-body.tsx` / `tasks/task-row.tsx` | Selection and bulk actions are explicitly false/empty at `signals/signal-collection-adapter.tsx:439-447`; no selection column in `signals/signal-table-presentation.tsx:36-88` | **CONVERGED** | Both collections are honest: selection scaffolding removed from Tasks until a real bulk action ships (OD-REDESIGN-83.2). |
