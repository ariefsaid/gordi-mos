...
- spec: FIX-THEN-SHIP — spec-reviewer (2026-06-26). All FR-001..022 + NFR-001..005 implemented; AC-001..007 (unit) + AC-010..013 (pgTAP 51/58) present & green. Gaps: **AC-020 e2e deferred** (full add→rename→archive→picker journey live-verified by Director via Playwright + screenshots; e2e needs admin-seeded user, tracked as follow-up). AC-014 (archived leaves picker) covered by existing `objectives/work-lines.test.ts` archived-filter asserts + `cascade-d1` FR-235 name-resolution + live verify.
...
Deploy prerequisite: apply migration 20260626000003. Follow-up: AC-020 e2e journey; AdminRoute↔RequireAccessRole consolidation.
