...
- design: BLOCK → fix-wave → re-review pending — design-reviewer (opus), 2026-07-17 rendered 4-lens. Critical: StartRunControl due-list floods /work/tasks (row per startable team, buries the table, renders on every group). Important: assign surface never names the step; phone occurrence group drops roll-up + assign affordance (Rule 9); provenance line ('via <role> on shift') from the owning convergence mockup not rendered (OD-65); 'Start run'/'Mulai proses' vocabulary (Rule 12/OD-58). Minor: 'N to assign' stutter. Operator core + IA + tokens judged strong; both-fronts manager 4/10 / operator 7/10 pre-fix.
