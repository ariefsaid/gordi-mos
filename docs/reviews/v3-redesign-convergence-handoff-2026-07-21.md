...
| Home / People / More (restart) | Claude sonnet (second Director session) | `.claude/worktrees/v3-hpm` / `v3/hpm` | ACTIVE; owner-fast-path | Prior codex WIP not found (4380 missing, aaca clean) — restarted from integrated tip; Home retry/projection duplication, People ViewTabs keyboard, More focus return, legacy /tasks links |
