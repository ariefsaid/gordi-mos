...
| X-4 | `FR-022` requires a written note whenever qty ≠ plan, blocking Submit on a fast capture field | FR-022 (specced) | **OPEN — owner call.** Owner's framing (*"production is according to plan, and can be checked who inputs by the planner"*) points at review-based accountability instead. Untouched. |
