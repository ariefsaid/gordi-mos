...
`set_updated_at` trigger, enable+FORCE RLS, **no DELETE grant**, `SECURITY INVOKER` guard). The
approval write path is one `SECURITY DEFINER` RPC (`ops.approve_kitchen_log`) that atomically mints
the `batch_id`, recomputes `kitchen_stock`, enqueues the `integrations.esb_push` row, and writes the
Daily Log summary mirror — the single audited multi-write point (FR-044/050/062/070/090). It stacks
on the access-role layer (`shared.has_access_role`, PR #41) for the member/ops_lead RLS gates.
...
- **Spec:** `docs/specs/kitchen-module.spec.md` (FR-001..101, AC-001..094; the DB-relevant ACs this
  plan owns are AC-001..008, AC-043, AC-070, AC-071 — all pgTAP). Status: Draft; the load-bearing
  owner scoping decisions **OD-K-1..4 are ratified** (`docs/decisions.md`) and the architecture is
  fixed by ADR-0010/0011/0012, so this plan conforms, it does not reopen.
...
- **Stacks on (hard build deps — must merge first):**
  - **PR #41 — access-roles DB substrate** (`feat/access-roles`, `20260619000001..03`). Kitchen's RLS
    calls `shared.has_access_role('ops_lead'/'admin')`, which that PR adds. **Kitchen's migrations
    will not resolve until #41's `…000002` (helpers) is applied.** The kitchen test seed (T-090)
    reuses `mos._test_seed_access_roles()` from #41's `…000003`.
  - The committed `supabase/migrations/20260612000004_ops_log_entries.sql` (`ops.log_entries`, whose
    `origin` CHECK this plan widens — FR-095/AC-071).
...
- **KQ-3 — Summary mirror grain (§3.6).** Decision: **one summary row per approved batch**
  (not per log). A batch is the unit the manager cares about ("Production: 12 portions Nasi Goreng
  approved"). The mirror write is idempotent per batch via a `unique (origin, detail->>'batch_id')`
  partial index (FR-092). Resolves §3.6 "eng-planner picks the grain".
...
- **KQ-5 — `batch_id` collision-safety mechanism (FR-051).** Decision: **a per-(prefix, date)
  counter table `ops.kitchen_batch_seq` + `SELECT … FOR UPDATE` inside the RPC.** The RPC locks the
  `(prefix, log_date)` seq row, reads `last_n`, increments, mints, and the unique constraint on
  `ops.kitchen_logs.batch_id` is the backstop. (Sequences can't encode the `<PREFIX>-YYYYMMDD-NNN`
  format; a gapless-in-rollback sequence would over-allocate. The seq-row lock is held for the
  sub-millisecond mint, so contention is negligible at kitchen's throughput.)
- **KQ-6 — `target_env` stamping (FR-080/081).** Decision: a `SECURITY INVOKER STABLE` helper
  `integrations.current_esb_target_env()` reads a `request.jwt.claims` / GUC the worker + the RPC
  both honor; **default `'dry_run'`**, and the pre-flip deployment sets it to `'goo'`. The RPC stamps
  `target_env := integrations.current_esb_target_env()` on enqueue — the app tier **cannot** pass
  `'gkid'` (NFR-001). The flip (OD-K-2) is a deployment-config change, not code (FR-082).

## 0.2 — Migration sequence numbers

Next after `20260612000006_ops_log_guard.sql`. Dated **2026-06-20** with a per-day counter from `01`
(the access-roles suite used 2026-06-19; kitchen is a fresh day to keep the two substrates' files
lexically separable):
...
- `20260620000008_ops_kitchen_logs_rls.sql` — kitchen_logs enable+FORCE RLS + policies + the
  `_guard_kitchen_log` trigger (status-transition immutability).
...
pgTAP files numbered **`36..47`**, continuing after the access-roles suite's `35_access_role_seed.sql`:
...
2. **k1-b outbox + helpers** — esb_push + config exposure + the two helpers (T-005, T-050..T-054).
...
> claims set via `set local request.jwt.claims`. **FIXED UUIDs** throughout (`…0d0N` people, `…0b0N`
  business units, `…0a1` org A — same fixtures as the access-roles suite).
...
- Verify: covered by T-011-test (the pgTAP AC-006 assertion in `36_wip_items.sql`).
...
- `plan(5)`. Seed `mos._test_seed_role_tree()` + `mos._test_seed_access_roles()` + `mos._test_seed_kitchen()`
  ...
  select is((select count(*)::int from ops.wip_items where flag_active and org_id = '00000000-0000-0000-0000-0000000000a1'),
            1, 'AC-006: member reads the seeded active WIP item');
  select throws_ok($$
  ...
  -- ops_lead (…0d3 holds admin in the access-roles fixture; reuse an ops_lead holder below).
  ...
  select lives_ok($$
    ...
    values ('00000000-0000-0000-0000-0000000000a1','Nasi Goreng','BOM-001','PD-PORSI-001')
...
- File: `supabase/migrations/20260620000002_ops_kitchen_plans.sql`.
  ...
  create table ops.kitchen_plans (
    ...
    notes         text,
    plan_by       uuid not null references shared.people(id) on delete set null
...
- `plan(4)`. Seed. As ops_lead, insert a plan row for (org A, 2026-06-20, WIP-1, Production) qty 10;
  ...
  select lives_ok($$
    ...
    values ('00000000-0000-0000-0000-0000000000a1','2026-06-20','00000000-0000-0000-0000-00000000w001','Production',10)
  $$, 'FR-031: ops_lead inserts a plan row');
  select throws_ok($$
    insert into ops.kitchen_plans (org_id, log_date, wip_item_id, action_type, qty_porsi)
    values ('00000000-0000-0000-0000-0000000000a1','2026-06-20','00000000-0000-0000-0000-00000000w001','Production',12)
  ...
  select lives_ok($$
    update ops.kitchen_plans set qty_porsi = 12
     where org_id='00000000-0000-0000-0000-0000000000a1' and log_date='2026-06-20'
       and wip_item_id='00000000-0000-0000-0000-00000000w001' and action_type='Production'
  $$, 'FR-031: ops_lead patches the plan qty (upsert UPDATE)');
  set local request.jwt.claims = '{"org_id":"00000000-0000-0000-0000-0000000000a1","person_id":"00000000-0000-0000-0000-0000000000d1","access_roles":["member"]}';
  select throws_ok($$
    insert into ops.kitchen_plans (org_id, log_date, wip_item_id, action_type, qty_porsi)
    values ('00000000-0000-0000-0000-0000000000a1','2026-06-20','00000000-0000-0000-0000-00000000w001','Production',5)
...
- File: `supabase/migrations/20260620000003_ops_kitchen_logs.sql`.
  ...
  create index kitchen_logs_batch_idx      on ops.kitchen_logs (batch_id) where batch_id is not null;
...
- Verify: table + CHECKs proven by `38_kitchen_logs_assign.sql` (T-041-test).

### T-041 — [green] enable+FORCE RLS + policies + status-transition guard on `ops.kitchen_logs` (FR-044, AC-001..004, AC-043)
- File: `supabase/migrations/20260620000008_ops_kitchen_logs_rls.sql`.
  ...
  -- Guard: the status transition is the GIGO gate. A member may NOT flip status out of Submitted;
  -- only ops_lead/admin may transition to Approved/Rejected. review fields (review_note/reviewed_by/
  -- reviewed_at/batch_id/posted_*) are worker/RC-only on the Approved path. SECURITY INVOKER (reads
  -- only current_person_id + has_access_role claim helpers; nothing to revoke — lint clean), mirrors
  -- ops._guard_log_entry / shared._guard_person_access_roles.
  ...
  comment on function ops._guard_kitchen_log() is
    'Guard (FR-044): Submitted→Approved/Rejected is ops_lead/admin only; submitted_by + Submitted-key immutable. SECURITY INVOKER.';
...
- Verify: AC-001..004, AC-043 green in `40_kitchen_logs_rls_gates.sql`.
...
- `plan(3)`. Seed. As member `…0d1` (org A, BU Kitchen-and-Bar `…0b01`):
  ```sql
  set local role authenticated;
  set local request.jwt.claims = '{"org_id":"00000000-0000-0000-0000-0000000000a1","person_id":"00000000-0000-0000-0000-0000000000d1","access_roles":["member"]}';
  ...
  select lives_ok($$
    ...
    values ('00000000-0000-0000-0000-0000000000b1','2026-06-20','Production','00000000-0000-0000-0000-00000000w001',12)
  $$, 'AC-001: member inserts own Submitted log');
  select is((select status from ops.kitchen_logs where wip_item_id='00000000-0000-0000-0000-00000000w001' and log_date='2026-06-20'),
            'Submitted', 'AC-001: default status Submitted');
  select is((select org_id from ops.kitchen_logs where wip_item_id='00000000-0000-0000-0000-00000000w001' and log_date='2026-06-20'),
...
- `plan(2)`. Seed. As member `…0d1`:
  ...
  select throws_ok($$
    ...
    values ('00000000-0000-0000-0000-0000000000b1','2026-06-20','Production','00000000-0000-0000-0000-00000000w001',5,'00000000-0000-0000-0000-0000000000d2')
  ...
  select throws_ok($$
    ...
    values ('00000000-0000-0000-0000-0000000000b1','00000000-0000-0000-0000-0000000000b1','2026-06-20','Production','00000000-0000-0000-0000-00000000w001',5)
...
- `plan(5)`. Seed + insert a Submitted log as service_role for `…0d1`. Then:
  ```sql
  ...
  select throws_ok($$
    update ops.kitchen_logs set status='Approved', reviewed_by='00000000-0000-0000-0000-0000000000d1', reviewed_at=now()
     where status='Submitted' and log_date='2026-06-20'
  $$, '42501', null, 'AC-003: member approve denied (guard)');
  -- AC-043: ops_lead approves; reviewed_by stamped.
  set local request.jwt.claims = '{"org_id":"00000000-0000-0000-0000-0000000000a1","person_id":"00000000-0000-0000-0000-0000000000d3","access_roles":["ops_lead"]}';
  -- (The real approve path is the RPC in T-060; here we prove the guard ALLOWS the direct transition
  --  for ops_lead so the RLS authority is independent of the RPC.)
  select lives_ok($$
...
- File: `supabase/migrations/20260620000005_ops_kitchen_batch_seq.sql`.
  ...
  -- SELECTs the row FOR UPDATE, reads last_n, increments, mints '<PREFIX>-YYYYMMDD-NNN'. The lock is
  -- held for the sub-ms mint; the unique(batch_id) on kitchen_logs is the collision backstop.
  ...
  comment on table ops.kitchen_batch_seq is
    'Per-(org, prefix, date) batch_id counter (FR-051). RPC locks FOR UPDATE, increments, mints. Collision-safe.';
...
- File: `supabase/migrations/20260620000006_integrations_esb_push.sql`.
  ...
  -- (inserts a 'pending' row); the worker (a later plan) transitions pending→posted/failed.
  ...
  comment on table integrations.esb_push is
    'Module-agnostic ESB outbox (ADR-0012, FR-070). One row/batch. Unique dedup_key = (source_module, source_ref, target_env) — the double-post guard (AC-008).';
  ...
  ```
- **DOWN:** drop policies; `drop table integrations.esb_push cascade;`
...
### T-053 — [green] `ops.stock_available_for_date()` read helper (KQ-1, FR-023/061)
...
- The start-of-day cut (the logging availability basis). `SECURITY INVOKER STABLE` (reads only
  `ops.kitchen_logs` + `ops.kitchen_stock`; RLS scopes it to the caller's org). Production adds,
  Transfers subtract; only Approved; `log_date < D` (start-of-day, FR-061).
  ```sql
  ...
  -- deployment sets the 'goo' claim/GUC; 'gkid' is reached ONLY at the owner-gated flip (FR-082).
  ...
  comment on function integrations.current_esb_target_env() is
    'ESB target env stamped at enqueue (FR-080/081). Default dry_run; deployment sets goo; gkid only at the flip. SECURITY INVOKER.';
  ```
- **DOWN:** drop both functions.
- Verify: used by the RPC (T-060) + proven by `42_kitchen_stock_recompute.sql` (the availability read).

### T-060 — [green] the atomic approval RPC `ops.approve_kitchen_log` (FR-044/050/062/070/090, AC-010..013, AC-034, AC-060)
...
- One `SECURITY DEFINER` (revoke PUBLIC/anon/authenticated execute — NFR-009; grant to `authenticated`
  ...
  is the single audited write point); (3) mints `batch_id` via the seq lock (KQ-5); (4) flips status
  ...
  begin
    ...
    -- (7) summary mirror into ops.log_entries (FR-090/092). Idempotent per batch (partial unique idx, T-070).
  ...
  comment on function ops.approve_kitchen_log(uuid, text) is
    'Atomic approval (FR-044/050/062/070/090): mints batch_id, recomputes stock, enqueues outbox, writes Daily Log mirror. SECURITY DEFINER.';
  ...
  ```
- **DOWN:** revoke grants; drop function.
- Verify: AC-010..013/034/060 green in `45_batch_id_mint.sql` + `46_approve_rpc_atomicity.sql`.

### T-060-test-a — [red] pgTAP: batch_id mint per (prefix, date), distinct under concurrency, no mint on reject (AC-010..013)
- File (new): `supabase/tests/45_batch_id_mint.sql`.
- `plan(4)`. Seed + insert 3 Submitted Production logs (service_role) + 1 TR + 1 TB. Approve in order
  via the RPC as ops_lead; assert the minted ids: `PR-20260620-001`, `…002`, `…003`, `TR-…001`, `TB-…001`.
  Reject a 6th log (direct UPDATE status='Rejected' as ops_lead, allowed by the guard) → assert its
  `batch_id is null`.
  ```sql
  -- (insert 3 PR + 1 TR + 1 TB Submitted logs as service_role, ids p1..p5; p6 to be rejected.)
  set local role authenticated;
  set local request.jwt.claims = '{"org_id":"00000000-0000-0000-0000-0000000000a1","person_id":"00000000-0000-0000-0000-0000000000d3","access_roles":["ops_lead"]}';
  select is(ops.approve_kitchen_log('00000000-0000-0000-0000-0000000000k1'::uuid, null), 'PR-20260620-001', 'AC-010: first PR → 001');
  select is(ops.approve_kitchen_log('00000000-0000-0000-0000-0000000000k2'::uuid, null), 'PR-20260620-002', 'AC-010: second PR → 002');
  select is(ops.approve_kitchen_log('00000000-0000-0000-0000-0000000000k3'::uuid, null), 'PR-20260620-003', 'AC-010: third PR → 003');
  select is(ops.approve_kitchen_log('00000000-0000-0000-0000-0000000000k4'::uuid, null), 'TR-20260620-001', 'AC-013: first TR → TR-001');
  select is(ops.approve_kitchen_log('00000000-0000-0000-0000-0000000000k5'::uuid, null), 'TB-20260620-001', 'AC-013: first TB → TB-001');
  ...
  update ops.kitchen_logs set status='Rejected', reviewed_by='00000000-0000-0000-0000-0000000000d3', reviewed_at=now()
   where id='00000000-0000-0000-0000-0000000000k6'::uuid;
  select is((select batch_id from ops.kitchen_logs where id='00000000-0000-0000-0000-0000000000k6'::uuid), null,
...
- Verify: `supabase test db` → file 45 red until T-060; then green. (AC-011 concurrency is proven at
  the worker/integration layer in plan k2 — the FOR UPDATE on the seq row is the mechanism; here we
  prove the sequential minting is correct. Note this deviation in the plan's §4 AC-map.)

### T-060-test-b — [red] pgTAP: approval atomicity — stock recompute + enqueue + mirror in one call (AC-034, AC-060)
- File (new): `supabase/tests/46_approve_rpc_atomicity.sql`.
- `plan(5)`. Seed one Submitted Production log qty 12. Approve via RPC. Assert: (a) the log is
  Approved with batch_id + reviewed_by; (b) kitchen_stock for (date, item) = 12; (c) an esb_push row
  exists for the batch (status pending, dedup_key set); (d) one ops.log_entries row origin='kitchen';
  (e) re-calling the RPC for the same log raises (status no longer Submitted) — idempotency is on the
  *batch*, not the *log* (FR-092).
  ```sql
  set local role authenticated;
  set local request.jwt.claims = '{"org_id":"00000000-0000-0000-0000-0000000000a1","person_id":"00000000-0000-0000-0000-0000000000d3","access_roles":["ops_lead"],"app.esb_target_env":"goo"}';
  select ops.approve_kitchen_log('00000000-0000-0000-0000-0000000000k1'::uuid, null);
  select is((select status from ops.kitchen_logs where id='00000000-0000-0000-0000-0000000000k1'::uuid),
            'Approved', 'AC-034: log Approved');
  select is((select usable_qty from ops.kitchen_stock where wip_item_id='00000000-0000-0000-0000-00000000w001' and log_date='2026-06-20'),
            12::numeric, 'AC-034: stock recomputed to the approved net');
  select is((select count(*)::int from integrations.esb_push where source_ref like 'PR-20260620-%' and status='pending'),
            1, 'AC-060: outbox row enqueued (pending)');
  select is((select count(*)::int from ops.log_entries where origin='kitchen'),
            1, 'AC-060: one Daily Log summary mirror row');
  select throws_ok($$ select ops.approve_kitchen_log('00000000-0000-0000-0000-0000000000k1'::uuid, null) $$,
    'P0003', null, 'FR-092: re-approve same log raises (idempotency is per-batch, not per-log)');
  ```
- Verify: `supabase test db` → file 46 red until T-060 + T-070 (origin) + T-090 (fixture); then green.
...
- `plan(4)`. As service_role, insert + approve (via RPC) Production 12, TR −4, TB −3 for one item one
  day → assert stock = 5. Approve a 4th (Submitted Production 9) → stock = 14. Then approve Transfers
  exceeding production on a different item/day → assert negative preserved (not clamped).
  ```sql
  -- (approve PR+12, TR-4, TB-3 for item W2 on 2026-06-21 via the RPC, then assert.)
  select is((select usable_qty from ops.kitchen_stock where wip_item_id='00000000-0000-0000-0000-00000000w002' and log_date='2026-06-21'),
            5::numeric, 'AC-031: stock = +12 -4 -3 = 5 (net of approved)');
  -- (approve a Submitted PR +9 already in the fixture for W2/2026-06-21)
  select is((select usable_qty from ops.kitchen_stock where wip_item_id='00000000-0000-0000-0000-00000000w002' and log_date='2026-06-21'),
            14::numeric, 'AC-034: stock recomputed after the 4th approval (5 + 9)');
  -- AC-032: negative preserved (approve TB 100 for an item with 0 production).
  select is((select usable_qty from ops.kitchen_stock where wip_item_id='00000000-0000-0000-0000-00000000w003' and log_date='2026-06-22'),
            -100::numeric, 'AC-032: negative balance preserved (not clamped)');
  -- AC-031: a still-Submitted log does not count (the approved net excludes it).
  select is((select usable_qty from ops.kitchen_stock where wip_item_id='00000000-0000-0000-0000-00000000w002' and log_date='2026-06-21'),
            14::numeric, 'AC-031: a pending Submitted log does not change stock (GIGO gate)');
  ```
...
- **DOWN:** `drop index if exists log_entries_kitchen_batch_uidx;` then
  `alter table ops.log_entries drop constraint log_entries_origin_check;` then re-add the original
...
- `plan(3)`. Insert a `log_entries` row with `origin='kitchen'` → lives_ok; assert the CHECK admits
  ...
  select lives_ok($$
    ...
    values ('00000000-0000-0000-0000-0000000000a1','00000000-0000-0000-0000-0000000000b1','kitchen','production','t',now())
  $$, 'AC-071: origin=kitchen accepted');
  select throws_ok($$
    insert into ops.log_entries (org_id, business_unit_id, origin, event_type, title, occurred_at)
    values ('00000000-0000-0000-0000-0000000000a1','00000000-0000-0000-0000-0000000000b1','bogus','production','t',now())
  $$, '23514', null, 'AC-071: out-of-set origin still rejected');
  select lives_ok($$
    insert into ops.log_entries (org_id, business_unit_id, origin, event_type, title, occurred_at)
    values ('00000000-0000-0000-0000-0000000000a1','00000000-0000-0000-0000-0000000000b1','kitchen_app','production','t',now())
...
- Seeds WIP items + BUs + Submitted/Approved logs the pgTAP files reference. Fixed UUIDs:
  `…0w001..03` WIP items, `…0k1..6` kitchen_logs, `…0b01` Kitchen-and-Bar BU. Inserts via service_role
  (bypasses RLS + guard). Mirrors `mos._test_seed_access_roles()` (PR #41).
  ```sql
  ...
  as $$
  begin
    -- Kitchen and Bar BU for org A (if absent).
    insert into shared.business_units (id, org_id, name)
    values ('00000000-0000-0000-0000-0000000000b1','00000000-0000-0000-0000-0000000000a1','Kitchen and Bar')
    ...
    values
      ('00000000-0000-0000-0000-00000000w001','00000000-0000-0000-0000-0000000000a1','Nasi Goreng','Mains',true,'BOM-001','PD-PORSI-001'),
      ('00000000-0000-0000-0000-00000000w002','00000000-0000-0000-0000-0000000000a1','Ayam Bakar','Mains',true,'BOM-002','PD-PORSI-002'),
      ('00000000-0000-0000-0000-00000000w003','00000000-0000-0000-0000-0000000000a1','Es Teh','Drinks',true,'BOM-003','PD-PORSI-003')
    on conflict (id) do nothing;

    -- Submitted kitchen logs for 2026-06-20 + 2026-06-21 + 2026-06-22 (org A, BU …0b1).
    -- k1..k3: Production; k4: TR; k5: TB; k6: Production (to be rejected). W2 day 2026-06-21 set in-file.
    insert into ops.kitchen_logs (id, org_id, business_unit_id, log_date, action_type, wip_item_id, qty_porsi, status, submitted_by)
    values
      ('00000000-0000-0000-0000-0000000000k1','00000000-0000-0000-0000-0000000000a1','00000000-0000-0000-0000-0000000000b1','2026-06-20','Production','00000000-0000-0000-0000-00000000w001',12,'Submitted','00000000-0000-0000-0000-0000000000d1'),
      ('00000000-0000-0000-0000-0000000000k2','00000000-0000-0000-0000-0000000000a1','00000000-0000-0000-0000-0000000000b1','2026-06-20','Production','00000000-0000-0000-0000-00000000w001',8,'Submitted','00000000-0000-0000-0000-0000000000d1'),
      ('00000000-0000-0000-0000-0000000000k3','00000000-0000-0000-0000-0000000000a1','00000000-0000-0000-0000-0000000000b1','2026-06-20','Production','00000000-0000-0000-0000-00000000w001',5,'Submitted','00000000-0000-0000-0000-0000000000d1'),
      ('00000000-0000-0000-0000-0000000000k4','00000000-0000-0000-0000-0000000000a1','00000000-0000-0000-0000-0000000000b1','2026-06-20','Transfer to Radiant','00000000-0000-0000-0000-00000000w001',4,'Submitted','00000000-0000-0000-0000-0000000000d1'),
      ('00000000-0000-0000-0000-0000000000k5','00000000-0000-0000-0000-0000000000a1','00000000-0000-0000-0000-0000000000b1','2026-06-20','Transfer to Bungur','00000000-0000-0000-0000-00000000w001',3,'Submitted','00000000-0000-0000-0000-0000000000d1'),
      ('00000000-0000-0000-0000-0000000000k6','00000000-0000-0000-0000-0000000000a1','00000000-0000-0000-0000-0000000000b1','2026-06-20','Production','00000000-0000-0000-0000-00000000w001',2,'Submitted','00000000-0000-0000-0000-0000000000d1')
    on conflict (id) do nothing;
  end;
  $$;
  comment on function mos._test_seed_kitchen() is
    'TEST-ONLY fixture (SECURITY DEFINER): kitchen WIP items + BUs + Submitted logs on the WU-A tree. Call after _test_seed_role_tree() + _test_seed_access_roles().';
  revoke execute on function mos._test_seed_kitchen() from public, anon, authenticated;
  ```
- **DOWN:** drop function. (The seeded rows live inside the pgTAP `begin;…rollback;` — no cleanup needed.)
- Verify: every pgTAP file (36..47) calls `select mos._test_seed_kitchen();` after the two prior seeds.
...
- Verify (exact): from the worktree root,
  ...
  ```
  Expect all files `00..47` green (the new `36..47` plus the unchanged prior suite + the access-roles
  `29..35`, **which requires PR #41 to be merged first** — see §0 deps). Confirm no regression in
...
- **PR-k1:** `supabase db reset && supabase test db` → all pgTAP files `00..47` green (new `36..47`
  prove AC-001..008, AC-031/032/034, AC-043, AC-060, AC-070, AC-071). Migrations reversible (every
  new migration carries a `-- DOWN:` block). `config.toml` exposes `integrations` (T-005).
...
- **Gating security review (NFR-011, ADR-0010 D11) — pre-merge, before any exposure/rollout.** The
  ...
  (`current_esb_target_env` reads a deployment GUC/claim, never a client value — FR-081/NFR-001);
  (3) the RPC's widened DEFINER write across `kitchen_logs`/`kitchen_stock`/`esb_push`/`log_entries`
  (all org-scoped by the loaded `v_log.org_id`, never a client-supplied org); (4) the dedup
  uniqueness + idempotent enqueue/mirror (AC-008, FR-092); (5) the no-DELETE posture (AC-005).
  **No production exposure until this passes + the owner flips the deploy flag.**
- **AC → task coverage map** (every DB-layer AC owned by exactly one task):

  | AC | Layer | Task(s) |
  |---|---|---|
  | AC-001 | pgTAP | T-041 / T-041-test-a |
  | AC-002 | pgTAP | T-041 / T-041-test-a |
  ...
  **Deferred to later plans (NOT this one):** AC-011 (concurrency — worker plan k2), AC-020..024
  ...
  AC-050..056 (worker — plan k2), AC-061 (mirror shape — UI plan k3), AC-070 (history migration —
  plan k4), AC-090..094 (e2e — plan k3/k4).

---

## 4. Deviations + open items for the Director / owner / auditor

1. **AC-011 (concurrent batch_id mint) is deferred to plan k2.** The mechanism (FOR UPDATE on the seq
   row, T-043/T-060) is built here, but a genuine concurrency proof needs two simultaneous sessions,
   which `supabase test db` (serial pgTAP) cannot express. Plan k2's worker integration tests prove it.
   Flagged so the AC-map is honest.
2. **The approval RPC is the single audited write point — deliberate concentration of DEFINER.** It
...
3. **`target_env` claim/GUC name (`app.esb_target_env`).** The exact claim key the worker sets at
   deploy is a deploy-time concern (plan k2 + the deploy plan). This substrate reads it via
   `current_setting('app.esb_target_env', true)` with a `dry_run` default — fail-safe. If the deploy
   uses a different key, T-053's function is the one line to change.
4. **Spec §10 open questions carried forward (not closed by this plan):** Q2 (migration timing — plan
   ...
   start-of-day (KQ-1).
5. **`ops.log_entries` partial-unique-index idempotency uses `detail->>'batch_id'`.** The RPC writes
...
- **The `ops.wip_items` ESB-identity columns are nullable** (master data may predate known ESB IDs).
