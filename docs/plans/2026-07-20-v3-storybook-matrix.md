...
- Preserve the existing `mos-app/package.json` `test`, `test:coverage`, and `test:watch` scripts, the existing `mos-app/vite.config.ts` Vitest block, its include/exclude behavior, and the 2,868-test Vitest 3.2.6 baseline.
...
- [ ] Add red fixture checks that call `validateStorybookMatrix` on a copy with one required story
  ...
  each must return a named failure. Add a scope-claim fixture proving a manifest that says
...
- [ ] Make `buildStorybookMatrix(repoRoot)` statically inspect each story source and
  ...
  metadata object with `jobs`, `states`, `responsive`, `canonicalImports`, and `scope` fields.
...
- [ ] Reject missing canonical imports, story/export markers, state entries, responsive entries,
  ...
  application migration or representative rendered acceptance.
...
**Interfaces:** The package script invokes `scripts/run-v3-storybook-tests.mjs`; that wrapper invokes `mos-app/node_modules/.bin/test-storybook`, never `npm run test-storybook`.
...
- [ ] Add `scripts/run-v3-storybook-tests.mjs`; start Storybook on a temporary port, wait for the
  ...
  `mos-app/node_modules/.bin/test-storybook --url <url> --maxWorkers=1`. Never call
...
**Produces:** An after-report equal to the 2,868-test before baseline with unchanged test discovery/configuration.
...
- [ ] Parse the after report with the same Node command and assert `numTotalTests === 2868`, zero
  failures, and the same suite count as the before report. Assert that `npm test` did not discover
  any `*.stories.tsx` file and that the Vite test block's include/exclude values are unchanged.
- [ ] Run `npm run test:coverage -- --reporter=dot` as the existing coverage command, verifying no
  Storybook-only test files are silently added to application coverage. If the existing baseline
  count or config behavior changes, stop and fix the integration or request Director review; do not
  update the baseline to make the regression disappear.
...
- [ ] Run `npm test`; expected: the unchanged 2,868-test Vitest 3.2.6 baseline is green.
...
- [ ] The implementation never changes `mos-app/vite.config.ts`'s Vitest project, test scripts, or
  include/exclude behavior; the 2,868-test regression proof occurs before and after package edits.
