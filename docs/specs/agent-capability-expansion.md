...
| **Typed widget results** (`data_table`/`data_chart`/`data_insight`; twice-validated; registry-rendered) | ADR-0045 §1; `QUERY_ENTITY_SCHEMA.as:"table"` | **MISSING** | P3 plan §1: "STAY (deferred) — P3c/late item"; `QUERY_ENTITY_SCHEMA` has no `as` field; no `WidgetSlot`/registry in panel |
...
| Capability | PMO evidence | MOS status | MOS evidence |
|---|---|---|---|
| **Safe markdown rendering** (`react-markdown`+`remark-gfm`; no raw HTML; `urlTransform`; user bubble stays literal) | ADR-0049 | **MISSING** (by design) | `AssistantPanel.tsx:12` "Plain-text only (FR-P2-AP-004): replies render via `{text}` — never dangerouslySetInnerHTML"; no `react-markdown`/`remark-gfm` in `package.json`; prompt ends "respond in plain text" |
| **Layered system prompt** (charter + tool-index + progressively-disclosed skills; removes "respond in plain text"; tool-index gated to registered tools) | ADR-0050; `prompt.ts` `buildAgentSystemPrompt(...,opts)` 4-layer builder | **MISSING** | MOS `prompt.ts` is a **flat grounding-only** builder (no `opts`, no tool index, no skills); ends "respond in plain text" |
