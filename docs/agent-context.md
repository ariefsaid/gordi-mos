...
| **Design reviews (rendered)** | `docs/reviews/design-mvp-push-2026-07-07.md` (4-lens, render-verified) · **UI-coherence audit IN FLIGHT (task #19 — see status doc)** |
...
- **⚠️ READ the `## Current focus (2026-07-07 late)` block in `docs/platform-workstream-status.md` first** — full state + branch map + ▶ NEXT. Summary: `dev`=**`c145e2a`**. **Pre-F hardening A1–A6 DONE + merged** (`docs/reviews/feat-harden-round2.md`; pgTAP 570 · unit 2345 · cov 95.43%). **Nav catalog FR-424** (Objectives/Projects&Processes back as capability-gated Work rail entries). **MVP-push design review render-verified** (`design-mvp-push-2026-07-07.md`). **CURRENT = UI-coherence polish (task #19)** — owner: "feels like several apps, no IxD convention, bleeds"; coherence audit IN FLIGHT; hypothesis = DESIGN.md already has the conventions → retrofit divergent modules. **Deputy** = text-only by design (`FR-P2-AP-004`), battery IS ported → fold in **C2 markdown + C3 typed-widgets** (`docs/specs/agent-capability-expansion.md`). **F (task #16, owner-gated)** = promote + deploy + secrets + backup drill + prod security gate + A4 job-GUC org-scope.
