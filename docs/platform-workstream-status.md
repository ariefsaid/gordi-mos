...
launcher all viewports; "Log"→"Kitchen Log"; breadcrumb self-crumb collapse). Select safe-swap + `RI-IXD-5` guard now
landed on this branch as WIP; remaining ledger items start with Follow-ups rebuild + deputy C2/C3.
...
IA C1–C4. **Retrofit plan (ledger §F, highest-leverage first):** (1) rebuild Follow-ups onto the kit · (2) **DONE on branch:**
kill the safe raw dropdowns w/ tokened `Select` `[NEW DESIGN.md primitive]` + guard · (3) Kitchen→shared toolbar+DataTable+state-kit + fix floating-bar
