...
launcher all viewports; "Log"→"Kitchen Log"; breadcrumb self-crumb collapse). Select safe-swap + `RI-IXD-5` guard and
Follow-ups rebuild-to-kit now landed on this branch as WIP; remaining ledger items start with Kitchen retrofit + deputy C2/C3.
...
rebuild Follow-ups onto the kit · (2) **DONE on branch:** kill the safe raw dropdowns w/ tokened `Select` `[NEW DESIGN.md primitive]` + guard · (3) Kitchen→shared toolbar+DataTable+state-kit + fix floating-bar
