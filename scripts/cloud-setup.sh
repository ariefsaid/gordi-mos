...
#   - Plugins declared in .claude/settings.json (superpowers) auto-install at session start.
#     Skills are NOT in git, so this script vendors them.
#
# ⚠ NOT YET RUN IN A REAL CLOUD SESSION. No Anthropic/Supabase published example of `supabase start`
# in a cloud sandbox exists — we are first. Each step below is individually proven (it mirrors
...
# The design battery needs design-review / impeccable / taste / ui-ux-pro-max. superpowers is a
# PLUGIN and is NOT vendored here: .claude/settings.json declares it, and the cloud installs
# repo-declared plugins at session start.
...
cat <<'EOF'
  ...
  Then read: docs/plans/CLOUD-AGENT-HANDOFF.md  (STEP 0 first — the base is not closed)
EOF
