...
# Re-entrancy: if this process tree ALREADY holds the test lock (flock-run.sh
# exported MOS_TEST_LOCK_HELD=1 into our env), run the command directly —
# re-acquiring our own advisory lock would self-deadlock.
if [ "${MOS_TEST_LOCK_HELD:-}" = "1" ]; then
  exec "$@"
fi
