...
w("")
w("> **GENERATED COVERAGE INVENTORY — do not hand-edit lifecycle state.** Rendered from")
w("> `docs/audits/surfaces.json` by `scripts/audit-register.sh render`. Mutate lifecycle state with")
w("> `audit-register.sh {bump|lock}`; read staleness with `scripts/audit-staleness.sh`. This remains")
w("> evidence for the coverage decisions named in `CLAUDE.md` / `AGENTS.md`; it is not a default")
w("> whole-matrix UI acceptance gate. DUE, NOT-REVIEWED, and due-axis notes are evidence and triggers")
w("> for a selected milestone or surface. Use `docs/takeover/mvp-ui-continuation.md` for current")
w("> bounded UI work (`pre-merge-check.sh` never existed in CI, DD-WAY-31).")
w("")
...
w("insufficiency / an explicitly selected milestone) — so \"what else is un-audited?\" is a query,")
w("not a discovery.")
