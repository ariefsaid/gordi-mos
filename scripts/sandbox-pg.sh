#!/usr/bin/env bash
# sandbox-pg.sh — stand up the pgTAP suite WITHOUT Docker.
#
# WHY THIS EXISTS: some sandboxes cannot pull the supabase/postgres Docker image (container-registry
# blob CDNs are blocked at the egress proxy), so `supabase start` / `supabase test db` do not work
# there. This script reproduces just enough of the real Supabase Postgres image on the SYSTEM
# PostgreSQL (apt: postgresql-16 + postgresql-16-pgtap) to apply supabase/migrations/*.sql and run
# supabase/tests/*.sql (pgTAP) via pg_prove. It does NOT run the auth/API containers — this is the
# DB-only path (pgTAP), not a logged-in app; for that, use scripts/cloud-agent-bootstrap.sh in a
# runtime that has Docker.
#
...
# ── 3. Supabase-compatibility bootstrap (roles, extensions, auth.* stub, ledger) ────
say "Applying bootstrap (roles / extensions / auth schema stub / migration ledger)"
psql_super -d "$DB" -f "$REPO/scripts/sandbox-pg-bootstrap.sql"

# ── 4. Migrations, in filename order (same order `supabase db reset` uses) ──────────
# Each applied file is also RECORDED in supabase_migrations.schema_migrations — the real CLI
# maintains that ledger, and supabase/tests/baseline_01_migration_set.sql asserts against it
# (applied count + no pre-squash version), so a sandbox without the ledger fails the suite.
...
for f in "${migrations[@]}"; do
  ...
  psql_super -d "$DB" -f "$f" >/tmp/sandbox-pg-last-migration.log 2>&1 || {
    ...
    die "migration failed: $(basename "$f") — fix the bootstrap shim, never the migration"
  }
  psql_super -d "$DB" -c "insert into supabase_migrations.schema_migrations (version, name) values ('$version', '$name') on conflict (version) do nothing;" >/dev/null
done
...
Run the pgTAP suite (equivalent of CI's \`supabase test db\`):
...
Or a single file:
  ...
  sudo -u postgres pg_prove -U postgres -d $DB --host /var/run/postgresql \\
    --ext .sql supabase/tests/mos_02_org_seam.sql

Re-run this script any time to reset to a clean, freshly-migrated+seeded state.
This does NOT start the auth/API containers or a logged-in app — DB/pgTAP only.
EOF
