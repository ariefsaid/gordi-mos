...
- **Audit coverage register (BLOCKING):** `docs/audits/REGISTER.md` (machine half `docs/audits/surfaces.json`) is the coverage authority — the enumerated surface × persona matrix carrying each surface's design generation, locked commit, pins, and never-audited (DUE) cells. A UI merge requires the touched surface **LOCKED** (its once-per-generation battery ran and pins landed) or explicitly **BUMPED** (a redesign lane is open); `scripts/pre-merge-check.sh` enforces this, `scripts/audit-register.sh {bump|lock}` drives the lifecycle, `scripts/audit-staleness.sh` reports FRESH/STALE-PINNED/DUE. The dimensions of quality and the check that owns each are enumerated in `docs/quality-model.md` (the review battery is the per-merge slice of it). Full design: `docs/plans/wise-discovering-frog.md`.
