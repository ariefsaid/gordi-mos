...
comment on column mos.work_lines.code is 'Stable authorization state for process gates; unlike name, code is not display metadata.';
...
as $$
  ...
  left join mos.work_lines wl on wl.org_id = b.org_id and wl.code = 'cafe_opening'
    and wl.type = 'process' and wl.archived_at is null
  left join mos.process_runs r on r.org_id = b.org_id and r.work_line_id = wl.id
  ...
  where b.org_id = shared.current_org_id()
    ...
    and shared.cafe_opening_can_start(b.id)
  order by b.name
$$;
comment on function mos.cafe_opening_branches() is
  'Branches the caller may start, including today''s run when already started.';
grant execute on function mos.cafe_opening_branches() to authenticated;
...
-- The UPDATE grant only lets the no-policy default-deny land as 0 affected rows instead of a
-- privilege error (integrations_05 asserts exactly that); the write control is the absence of the
-- policy, pinned by integrations_01's policy catalog.
...
-- $$;
-- -- Restore the process functions this migration replaced; rollback must preserve the prior gate.
-- create or replace function mos.spawn_process_run(p_work_line_id uuid, p_owning_team_id uuid, p_target_date date)
-- returns jsonb
-- language plpgsql
-- security definer
...
-- -- $$;
-- -- The final three drops belong to the DOWN section above, not the UP migration.
