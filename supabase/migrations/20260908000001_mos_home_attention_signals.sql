...
-- functions only, so DOWN is the two drops):
--   drop function if exists mos.home_attention_signals();
--   drop function if exists mos._is_lead_of_team(uuid, uuid);

-- Person-parametrized lead fact. mos.is_team_lead(p_team_id) stays untouched and continues to
-- answer only for the calling session; this helper answers the same question about an arbitrary
-- person, so the read function can also ask "did ANY lead of the owning Team already ack this
-- signal" — that acknowledgement is what removes the row from every lead's Needs you now.
--
-- Reads shared.person_access_roles (the persisted access-role table, source of truth) rather
-- than the JWT claim that mos.is_team_lead reaches through shared.has_access_role — the JWT is a
-- per-caller cache and cannot answer for a different person. The three arms are identical in
-- shape to mos.is_team_lead's three arms (20260906000001), so the two agree by construction
-- whenever p_person_id equals the calling person.
create or replace function mos._is_lead_of_team(p_person_id uuid, p_team_id uuid)
returns boolean
language sql
stable
security invoker
      ...
      and (
        ...
        or (
          ...
          and exists (
            ...
            where home_membership.person_id = p_person_id
              ...
              and (home_membership.is_primary or not exists (
                ...
                where primary_membership.person_id = p_person_id
        ...
        or exists (
          ...
          where head_pr.person_id = p_person_id
...
$$;
comment on function mos._is_lead_of_team(uuid, uuid) is
  'Person-parametrized lead fact for a Team (OD-WAY-96) — the shape of mos.is_team_lead with the '
  'session person swapped for p_person_id, reading shared.person_access_roles directly instead of '
  'the JWT. Used by mos.home_attention_signals() to detect that ANY lead of a Team has ack''d a '
  'Signal, which clears the row from every lead''s Needs you now.';
revoke execute on function mos._is_lead_of_team(uuid, uuid) from public, anon;
grant  execute on function mos._is_lead_of_team(uuid, uuid) to authenticated;
      ...
      (
        ...
        and not exists (
          ...
          where a.signal_id = s.id
            and mos._is_lead_of_team(a.person_id, s.owning_team_id)
        )
      )
      or (
        s.attention = 'Urgent'
