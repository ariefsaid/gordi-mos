...
insert into ops.kitchen_logs (id,org_id,business_unit_id,log_date,branch_id,activity,action,wip_item_id,qty_porsi,status,submitted_by,notes) values
 ...
 ('00000000-0000-0000-0000-00000000ac21','00000000-0000-0000-0000-0000000000a1','00000000-0000-0000-0000-00000000bb01',current_date,'00000000-0000-0000-0000-00000000bf02','bar','produce','00000000-0000-0000-0000-00000000ab03',9,'Submitted','00000000-0000-0000-0000-0000000000d6',null),
...
select is(shared.cafe_opening_team('00000000-0000-0000-0000-00000000bf02'),'00000000-0000-0000-0000-00000000cc02'::uuid,'AC-007: branch opening owner is kitchen stream');
select is((select count(*)::int from shared.team_memberships where person_id='00000000-0000-0000-0000-0000000000d6' and team_id='00000000-0000-0000-0000-00000000cc02'),1,'debug: membership inserted');
select ok(shared.can('process.start'),'debug: member has process.start');
select ok(mos.can_start_process_for_team('00000000-0000-0000-0000-00000000cc02'),'debug: member team gate');
select lives_ok($$select mos.spawn_process_run('00000000-0000-0000-0000-00000000c001','00000000-0000-0000-0000-00000000cc02',current_date)$$,'AC-007: kitchen member starts opening');
...
select lives_ok($$select mos.spawn_process_run('00000000-0000-0000-0000-00000000c001','00000000-0000-0000-0000-00000000cc03',current_date)$$,'AC-007: bar supervisor starts existing opening');
...
set local request.jwt.claims='{"org_id":"00000000000000000000000000000001","person_id":"00000000000000000000000000000001","access_roles":["member","finance"]}';
select is((select count(*)::int from mos.due_process_runs() where process_name='Café Opening'),0,'AC-009: finance sees no due opening');
set local request.jwt.claims='{"org_id":"00000000-0000-0000-0000-0000000000a1","person_id":"00000000-0000-0000-0000-0000000000d4","access_roles":["member","supervisor"]}';
select ok(ops.is_stream_reviewer('00000000-0000-0000-0000-00000000bf02','bar'),'AC-011: secondary stream membership reviews');
set local request.jwt.claims='{"org_id":"00000000-0000-0000-0000-0000000000a1","person_id":"00000000-0000-0000-0000-0000000000d4","access_roles":["member","supervisor"]}';
select lives_ok($$select ops.approve_kitchen_log('00000000-0000-0000-0000-00000000ac20',null)$$,'AC-012: off-plan log with submitter note accepts null reviewer note');
select throws_ok($$select ops.approve_kitchen_log('00000000-0000-0000-0000-00000000ac21',null)$$,'42501',null,'AC-012: off-plan log without either note is refused');
...
select lives_ok($$insert into ops.kitchen_plans(log_date,wip_item_id,branch_id,activity,action,qty_porsi) values(current_date,'00000000-0000-0000-0000-00000000ab03','00000000-0000-0000-0000-00000000bf02','bar','produce',4)$$,'AC-010: two-stream supervisor writes secondary plan');
...
select is((select count(*)::int from integrations.esb_push),2,'AC-013: ops lead reads outbox');
