...
select isnt((select foreign_id from signal_readback_probe), null,
  'JOURNEY foreign: create_signal_with_mentions returns an id with default attention');
...
select is((select attention from mos.signals where id = (select own_id from signal_readback_probe)), 'FYI',
  'JOURNEY control: omitted attention defaults to FYI');
