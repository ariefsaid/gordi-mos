...
select throws_ok($$ update shared.teams set produces = null where org_id = '00000000-0000-0000-0000-0000000000a1' and code = 'rumah_rames_kitchen' $$,
  '23514', null, 'AC-001: nulling produces on a stream Team is refused by the pair check');
...
select results_eq($$ select destination_branch_id from ops.allowed_kitchen_destinations('00000000-0000-0000-0000-0000000000a1'::uuid, '00000000-0000-0000-0000-00000000bf04'::uuid, 'bar'::text) $$, $$ values ('00000000-0000-0000-0000-00000000bf01'::uuid), ('00000000-0000-0000-0000-00000000bf03'::uuid), ('00000000-0000-0000-0000-00000000bf02'::uuid) $$, 'AC-003: Cikal bar has no intra-branch destination without a kitchen counterpart');
select results_eq($$ select destination_branch_id from ops.allowed_kitchen_destinations('00000000-0000-0000-0000-0000000000a1'::uuid, '00000000-0000-0000-0000-00000000bf01'::uuid, 'kitchen'::text) $$, $$ values ('00000000-0000-0000-0000-00000000bf04'::uuid), ('00000000-0000-0000-0000-00000000bf03'::uuid), ('00000000-0000-0000-0000-00000000bf02'::uuid) $$, 'AC-003: GHQ kitchen excludes itself and reaches every other stream branch');
...
select results_eq($$ select destination_branch_id from ops.allowed_kitchen_destinations('00000000-0000-0000-0000-0000000000a1'::uuid, '00000000-0000-0000-0000-00000000bf03'::uuid, 'bar'::text) $$, $$ values ('00000000-0000-0000-0000-00000000bf04'::uuid), ('00000000-0000-0000-0000-00000000bf01'::uuid), ('00000000-0000-0000-0000-00000000bf03'::uuid), ('00000000-0000-0000-0000-00000000bf02'::uuid) $$, 'AC-003: Radiant bar includes its own kitchen arm and every other bar branch');
...
select throws_ok($$ insert into ops.kitchen_plans (log_date, branch_id, activity, action, wip_item_id, qty_porsi) values ('2026-06-25','00000000-0000-0000-0000-00000000bf03','kitchen','produce','00000000-0000-0000-0000-00000000ab01',1) $$, '42501', null, 'AC-001: plan refuses a non-producing stream');
select throws_ok($$ insert into ops.kitchen_plans (log_date, branch_id, activity, action, destination_branch_id, wip_item_id, qty_porsi) values ('2026-06-25','00000000-0000-0000-0000-00000000bf02','kitchen','transfer','00000000-0000-0000-0000-00000000bf05','00000000-0000-0000-0000-00000000ab01',1) $$, '42501', null, 'AC-003: plan refuses a disallowed destination');
...
select is(
  (select coalesce(array_agg(c.relname order by c.relname), '{}')
     ...
     join pg_depend d on d.classid = 'pg_policy'::regclass
       and d.objid = p.oid and d.refclassid = 'pg_class'::regclass
     join pg_class referenced on referenced.oid = d.refobjid
    where c.relnamespace = 'ops'::regnamespace
      and c.relname in ('kitchen_logs','kitchen_plans','kitchen_stock','stream_completeness')
      and referenced.oid = 'shared.team_memberships'::regclass),
  '{}'::name[],
  'AC-005: no stream-bearing table policy has a dependency on shared.team_memberships');
select * from finish();
rollback;
