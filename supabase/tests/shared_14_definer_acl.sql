...
create temporary table definer_acl_authenticated_allowlist (
  schema_name name not null,
  function_name name not null,
  identity_arguments text not null,
  reason text not null
) on commit drop;
insert into definer_acl_authenticated_allowlist (schema_name, function_name, identity_arguments, reason) values
  ...
  ('mos', 'create_notification', 'p_owner uuid, p_severity text, p_title text, p_body text, p_metadata jsonb', 'authenticated notification RPC; follow-up must harden caller-supplied content'),
...
select ok(
  exists (
    ...
    where p.prosecdef
      ...
      and not exists (
        select 1
        from definer_acl_authenticated_allowlist a
...
select ok(
  not has_function_privilege('authenticated', p.oid, 'EXECUTE')
    or exists (
      select 1 from definer_acl_authenticated_allowlist a
      where a.schema_name = n.nspname
        and a.function_name = p.proname
        and a.identity_arguments = pg_get_function_identity_arguments(p.oid)
    ),
