...
begin
  ...
  values
    ...
    (gen_random_uuid(), v_org, 'Dial in new Brazil single-origin', bu_roast, 'In Progress',
       ...
       current_date - 4, now() - interval '2 days', null, p_dewi, now(), now()),
    (gen_random_uuid(), v_org, 'Update espresso recipe cards', bu_cafe, 'In Progress',
       ...
       current_date + 2, now() - interval '5 hours', null, p_dewi, now(), now()),
    (gen_random_uuid(), v_org, 'Photograph new pastry line', bu_kitchen, 'In Progress',
       ...
       current_date + 8, now() - interval '1 day', null, p_dewi, now(), now()),
    (gen_random_uuid(), v_org, 'Q3 wholesale price list', bu_sales, 'In Progress',
       ...
       current_date + 14, now() - interval '3 days', null, p_dewi, now(), now()),
    ...
    (gen_random_uuid(), v_org, 'Replace grinder burrs (Cafe 2)', bu_cafe, 'Blocked',
       ...
       current_date - 7, now() - interval '6 days', null, p_dewi, now(), now()),
    (gen_random_uuid(), v_org, 'Source compostable cups vendor', bu_fin, 'Blocked',
       ...
       current_date - 5, now() - interval '4 days', null, p_dewi, now(), now()),
    -- Open (3)
    (gen_random_uuid(), v_org, 'Plan barista latte-art workshop', bu_cafe, 'Open',
       ...
       current_date + 17, now() - interval '1 day', null, p_dewi, now(), now()),
    (gen_random_uuid(), v_org, 'Roastery extractor PM schedule', bu_roast, 'Open',
       ...
       current_date + 22, now() - interval '2 days', null, p_dewi, now(), now()),
    (gen_random_uuid(), v_org, 'Draft Q3 OKRs for cafe team', bu_cafe, 'Open',
       ...
       current_date + 25, now() - interval '7 hours', null, p_dewi, now(), now()),
    -- Done (2) — #752: one within the 7-day live-work window, one older, so My work / Team work
    -- shows only the recent stamp and All shows both. Seeded completed_at pass through the guard
    -- because seed runs as the service role (the completion clause is authenticated-scoped).
