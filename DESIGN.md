...
- **The Rail Type Ladder (DD-WAY-33).** The rail carries three levels, and each one is a distinct **rung** — a nav item's weight states its level, so the tree is legible without expanding anything. This closes the gap that let Money and Inbox, sitting after Work's children, read as part of the group above them.
  ...
  | **Child** | Tasks · Projects & Processes · Objectives · Signals · Events · Log · Plan · Stock · Review · Pushes | 13px (`--font-size-mono`, the ramp's dense-compact step — not a monospace face) / **500**, `muted-foreground`, **15px** icon, indented behind a **1px `border` hairline indent guide** set on the destination icon's centre line. |
  ...
  In the **icon-only compact rail** (920–1099.98px) the group overlines hide and the indent guide drops — there is no indent to guide — but the two item rungs stay, carried by their 17px/15px icon sizes. Every value in the ladder's rules comes from the type ramp or from the `--rail-*` geometry tokens; the rail stylesheet's pre-ladder blocks (tooltip, compact count badge, coarse-pointer targets) still carry raw px on the #425 ledger.
...
- **Inline segmented (`seg`):** 32px track on `secondary` (3px inset padding), options fill the track height (measures 26px, not the previously-stated 28px — corrected by live measurement, extract 2026-07-28), "on" = white `background` pill + `foreground` + 600 + `0 1px 2px` lift. Label size is the `mono` token's 13px *number* reused for sizing only — the face stays DM Sans (`font-family: inherit`), never the SF Mono typeface; this is an established v3 pattern (a token's numeric value borrowed for a non-typographic use), not a new exception.
  ...
  **⚠️ The rule is not met today, and the exception is live.** `dashboard/cut-toggle.css` is the only file that `@import`s it. `dashboard/window-selector.css` re-authors the whole grammar pixel-for-pixel under its own class family — 32px track, `--secondary`, 3px padding, 4px gap, `--radius-sm`, the same `calc(var(--radius-sm) - 2px)` inner corner and the same `0 1px 2px` lift — with no import. It is live and it sits *directly beside* CutToggle on Money's toolbar (the `7d / 30d / 60d / Custom` window control), which is exactly the adjacency the extraction existed to prevent. **Two live consumers, one importer.** Folding WindowSelector onto the shared file is real, open extraction debt.
