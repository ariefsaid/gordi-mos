...
**The Tinted-Status Rule.** Status is shown as an 8px colored dot plus a pill tinted at ~10–18% of the status hue with a darkened text variant — never a fully saturated solid fill behind body text. Solid status fills are reserved for the destructive *button* only. The Signal Urgent attention treatment has a separate 6px marker inside its surface-specific capsule; it does not redefine the shared status-pill dot. Every shared status chip keeps its text label (never dot-only) so status stays perceivable when grouping ≠ Status — see §5 Badges.
...
### Buttons
- **Shape:** 8px radius (`{rounded.sm}`, the control radius — unchanged in absolute px by OD-P3-10, now expressed as `calc(var(--radius) - 4px)`), 32px tall with border-box sizing, `6px 12px` padding, and a 7px gap to a 15px icon. The full-width `.btn-touch` variant uses `width: 100%`, `min-height: 44px`, and the body size. Icon-only controls use `IconButton`: 32px square at desktop, with a 44px phone floor except for its explicitly dense `small` variant.
...
- **Disabled (shipped):** `.btn:disabled` and `.btn[aria-disabled="true"]` use `opacity: 0.5`, `cursor: not-allowed`, no shadow, no background image, and `filter: saturate(0.35)`. The shared rule keeps disabled controls visibly inert without inventing a per-surface treatment.
...
### Badges / Status Pills
- **Status pill:** 22px tall, **8px `rounded.sm` radius (rounded-rect — ratified OD-REDESIGN-91 #30/E1)**, 12px/600 label, with a leading 8px colored `dot` (the dot itself stays circular, `rounded.full`). Background = status hue at ~10–18%, text = a darkened variant of the hue for AA contrast (applied via the named CSS token — see below). Variants observed: `open` (blue), `won` (green), `lost` (red), `overdue` (amber). Default/neutral badge uses `secondary` bg + `muted-foreground` text. Signal Urgent/Needs attention chips are a separate capsule treatment; Urgent's 6px marker is surface-specific. No gradient (status).
...
The darkened-AA text values for the four non-neutral pill variants are defined as named CSS custom properties. The `StatusPill` component applies them through CSS custom-property references as inline style values (`var(--status-…-text)`), keeping the theme-aware Display-P3 values live.
...
- **Source badge** (the business unit): calm, NOT a per-unit rainbow — only the two ops-writing
  units tint, everything else neutral. Kitchen and Bar = `primary/12%` + `--status-open-text`
  (the shared `<Pill>` tone, `Pill.css` — the "10%" this line carried for two years was never what
  shipped);
...
- **Disabled (shipped per field primitive):** `TextInput`, `Select`, `Picker`, and `DateField` use the `secondary` surface and a `not-allowed` cursor for disabled fields, with each primitive retaining its own muted/disabled affordance. A single cross-field opacity/text contract remains unratified, so consumers use the primitive's class rather than inventing a route-local style.
...
- **Row activation (A3, OD-WAY-94).** A click anywhere on a row — the title included — opens the record (OD-REDESIGN-63). Inline title editing starts from a hover/focus pencil, F2, or double-click; never from a single click. The footer legend states this grammar. A row overflow menu renders only when it holds two or more actions.
...
- **The Rail Type Ladder (DD-WAY-33).** The rail carries three levels, and each one is a distinct **rung** — a nav item's weight states its level, so the tree is legible without expanding anything. This closes the gap that let Money and Inbox, sitting after Work's children, read as part of the group above them.
  ...
  |---|---|---|
  | **Group overline** | `DESTINATIONS` · `RETAIL OPS` · `B2B OPS` | The § Typography **Overline** voice, unchanged: DM Sans 600, 11px (`--font-size-overline`), ls 0.06em, UPPERCASE, `muted-foreground`. `aria-hidden` — a visual divider, not a nav landmark. |
  | **Destination** | Home · Work · Money · Inbox · Café · Ecommerce · Roastery · Admin Settings · Personal Profile | 13.5px (`--font-size-control`) / **600**, `foreground`, **17px** stroke-2 icon. The rung a scanning eye lands on. |
  | **Child** | Tasks · Projects & Processes · Objectives · Signals · Log · Plan · Stock · Review · Pushes | 13px (`--font-size-mono`, the ramp's dense-compact step — not a monospace face) / **500**, `muted-foreground`, **15px** icon, indented behind a **1px `border` hairline indent guide** set on the destination icon's centre line. |

  The icon carries no colour of its own — it inherits its rung, so glyph and label move together. **Active is unchanged and outranks the rung at every level:** `blue3` tint + `--text-on-accent-tint` + 600. It is expressed as a compound selector (`.rail-item--dest.rail-item--active`) so it wins on specificity, never on source order.

  Two consequences the ladder makes binding. **The rail has exactly three rungs — no fourth.** Work's sub-family eyebrows (`Execution` · `Work systems` · `Direction` · `Cadence`) are deleted, not conditionally hidden; the family ORDER survives and governs how the children are sequenced. An eyebrow inside an already-indented child list is a level the ladder does not have. **And the ladder is per-level, not per-surface:** the phone "More" drawer wears the same three rungs from the same stylesheet — a destination must not demote itself to a child because the viewport shrank.

  In the **icon-only compact rail** (920–1099.98px) the group overlines hide and the indent guide drops — there is no indent to guide — and only destination roots render. Every value in the ladder's rules comes from the type ramp or from the `--rail-*` geometry tokens; the rail stylesheet's pre-ladder blocks (tooltip, compact count badge, coarse-pointer targets) still carry raw px on the #425 ledger.
...
- **Rail collapse is the user's, not only the viewport's (#442).** The icon-only rendering above was reachable only by shrinking the window into the 920–1099.98px band — at the widths where the rail is most worth trading for content, the user had no say. A **collapse toggle** now sits at the rail's top edge and turns that SAME compact rendering on. There is exactly one collapsed style; this is the switch, not a second look.
  ...
  | **Precedence** | **Width wins below 1100px.** In the compact band the labels do not fit — a measurement, not a taste — so a stored "expanded" cannot overrule it, and the control is **absent** there rather than present-and-inert. A toggle that visibly does nothing when pressed is a worse answer than no toggle. |
...
> **Access boundary state.** A route the viewer is admitted to reach but not authorised to use renders, inside the shell frame, the page head with its title and one quiet dashed panel: a padlock glyph · `<Area> is outside your access` · one sentence telling the viewer to ask their lead or an admin · one outline link back — to the owning destination when only a link inside an admitted destination was denied, otherwise `Back to Home`. The denial is stated once: the page head carries no sentence of its own. It never redirects silently and never names data the viewer may not see. Navigation is unchanged: rendered links still equal admitted routes (OD-WAY-51); the boundary is what a typed or shared URL meets.
...
- **Do** show status as an 8px dot + a tinted pill (status hue ~10–18% bg, darkened text), and reserve solid fills for the `destructive` button only. Keep surface-specific Signal Urgent markers at 6px inside their attention capsule.
...
**Reporting empty.** When a reporting page's source has no rows, the filter toolbar and view tabs do not render — a control that filters nothing is not a control — and the head carries no meta placeholder. Loaded, the head meta is one sentence: `<n> <cut noun> · as of <snapshot time> · latest reporting day <date>`. A figure tile carries its basis chip (`certified` · `interim`) and its as-of; a figure with no basis is absent, not `—`.
