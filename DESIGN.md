...
- **The Rail Type Ladder (DD-WAY-33).** The rail carries three levels, and each one is a distinct **rung** — a nav item's weight states its level, so the tree is legible without expanding anything. This closes the gap that let Money and Inbox, sitting after Work's children, read as part of the group above them.
  ...
  In the **icon-only compact rail** (920–1099.98px) the group overlines hide and the indent guide drops — there is no indent to guide — but the two item rungs stay, carried by their 17px/15px icon sizes. Every value comes from the type ramp or from the `--rail-*` geometry tokens; the rail's stylesheet holds no raw px, hex or hsl literal.
