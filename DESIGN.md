...
- **Decision rows.** In a queue where each row carries a decision, the filled primary is the batch action (`Approve all on-plan (N)`); per-row approve is `.btn-outline`, per-row reject is ghost, and no row control is ever filled — including after a decision moves focus. A confirmation that is not a decision (completeness) is a checkbox row at the foot of the queue.
