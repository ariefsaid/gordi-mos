...
| `scripts/sssf-config.test.sh` | MOS-authored (#336/#343/#357) | FR-007 roster checks: every slot pinned from the ruled substrate set, contracts under `agents/`, protected_files coverage, two-places header, (#343) every roster model has an attribution trailer row in `git_helper.py`, and (#357) the permission boundary proven on the runner's real `permissions.enforce()` in a scratch repo — a builder deliverable in general `scripts/` is allowed; edits to the verify gate, `adw_modules/`, and CI are rolled back and refused; a re-broadened `scripts/` entry (the #349 failure) and a control script losing protection each go red — every check proven able to fail against perturbed fixtures in the same run. |
