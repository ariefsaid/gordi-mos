...
| `adws/adw_modules/data_types.py` | PMO delta + MOS fix (#335/#336) | PMO: `contract:` key on the agent schema. MOS: `QualityArea`/`QualityOperation` Literals admit `"db"`/`"test"` — upstream's Literals would reject the pgtap block's spec at runtime (latent PMO bug: PMO ships the pgtap block against the unextended Literals). #350: `AuditFinding`/`SurfaceVerdict`/`AuditOutput` envelope types for the design-audit chain (per-surface verdicts, artifact-backed; no `commit_message` — the audit chain commits nothing). #386: `PiResult.provider_error` carries the provider's own account of a turn that settled with `stopReason: "error"`. |
| `adws/adw_modules/agents.py` | PMO delta + MOS addition (#335) | PMO: contract file read + verbatim append after prompt render. MOS: a declared-but-missing contract file refuses the run in `validate()` before anything spawns (FAC-002) — never a silent contract-less agent. #386: `ProviderFailure` — an empty response carrying a provider error is a transport fault, refused in `_parse_with_retries` before the JSON-correction attempts and never re-raised as "never produced valid JSON"; the message names the roster model and the upstream status verbatim. Emptiness ALONE still raises the parse error, so the two causes stay distinguishable in both directions. |
| `adws/adw_modules/agent_pi.py` | MOS fix (#386) | Records `provider_error` on the result: a `message_end` whose `stopReason` is `"error"` carries no content and its `errorMessage` is the only account of why (observed: `429: {"code":"1302", …}` from a rate limit, three auto-retries deep). pi's own auto-retries each emit a turn, so the LAST one is the settled state — a later clean turn clears the field. Leaving it in `raw_output.jsonl` is what let the caller blame the model for a request the provider refused. |
| `adws/adw_modules/quality.py` | PMO delta + MOS wiring (#335/#336) | Grown gate wired to MOS: typecheck → lint (script pins zero warnings) → unit suite → pgTAP-when-`supabase/`-touched, cheap blocks first, first failure skips the heavy ones; all app blocks run in `mos-app/`; unit suite behind `scripts/with-test-lock.sh`; pgTAP reset+test chained inside ONE `scripts/with-db-lock.sh` hold against this project's own stack (`supabase/config.toml` ports). `run_quality()` (the census used by adw_quality / adw_plan_build_test_quality) aligned to the same real blocks: cheap-first order + conditional pgTAP, still running everything on failure (collect-all is its contract). #389: captured streams are normalised at the edge — `TimeoutExpired` carries the RAW bytes read before the kill (`text=True` decodes only on the normal return) and the stream that produced nothing carries None, so concatenating them into `output_tail` killed the runner with `TypeError: can't concat str to bytes` and the timeout was never reported. `_text()` decodes utf-8/surrogateescape like `permissions.py`; the artifact is written back with the same handler so the log stays byte-exact; `_envelope_safe()` re-encodes the travelling tail through the original bytes because a lone surrogate cannot be serialized and would move the same crash into the next agent's envelope. The timeout is now a RESULT: partial output survives, and both the tail and the console line name the budget that killed the check rather than exit 124 alone. |
| `adws/adw_modules/permissions.py` | MOS fix (#357) | Rename-proof enforcement: `snapshot()`'s diff runs `--no-renames` — with rename detection on, a staged `git mv` of a protected file collapses both halves into one `dir/{old => new}` numstat pseudo-path that matches no protected pattern, letting an agent move a gate/config file aside untouched by enforcement. `_roll_back()` restores from HEAD instead of the index (a staged change has already tampered the index; index-restore would reinstate the agent's version, or fail on the staged-delete half of a rename), and a staged addition absent from HEAD is unstaged and deleted. Snapshot reads are NUL-separated (`-z` on the numstat and ls-files calls): without it git C-quotes any path holding a tab/quote/backslash/control byte, so the fingerprint key arrives quoted, matches no protected pattern, and the file survives enforcement; `-z` delivers paths verbatim (numstat record = `added TAB deleted TAB path NUL`, path parsed as everything after the second tab since a name may itself contain tabs). Git reads run in BYTES mode (`text=True` would apply universal-newline translation, rewriting a `\r` inside a filename to `\n` — matching and rollback then act on a name that does not exist and the real file survives); each path is decoded at the edge with utf-8/surrogateescape, round-trippable for non-UTF-8 names. On top of the exact representation chain, the class is capped wholesale: `_control_bytes()` makes any changed path containing a byte < 0x20 a breach by definition for every agent, pattern or no pattern — no legitimate path here carries one. Proven both ways in `scripts/sssf-config.test.sh` (rename + quoted/CR-path scenarios, each with a re-open-the-hole can-fail control: strip `--no-renames`, strip `-z`, force text-mode translation, strip the cap). |
...
| `adws/adw_simple_sdlc.py` | PMO delta + MOS rewiring (#335/#336/#343) | PMO: `--builder`/`--reviewer` roster parameterization (FR-006). MOS: `commit_plan` and `commit_docs` are TRACE-record phases, not commits — this repo is public with a blunt docs-split rule, so plan and write-up live in the session dir + trace only; the code commit (`commit_build`) is the chain's only commit. Module docstring describes the one-commit model. #343: `commit_build` passes the executing builder's roster model so the trailer attributes the model that built. Findings-rerun mode (#343 train): `--findings <text/file>` + `--from-adw-id <prior>` reuses the prior session's recorded plan envelope, skips the plan phase (a `reuse_plan` trace phase records the link), and enters at build with the findings as the prompt; new adw_id, prior id logged on the request record; round caps and gates unchanged. #367: `reuse_plan` COPIES the prior plan artifacts into the current session's `context_handoff` (two passes: EVERY source validated — prior-session containment + existence — before the first copy, so a refusal leaves the handoff untouched, never half-populated), so the reviewer finds `<context_handoff_dir>/plan.md` exactly where a normal run's planner leaves it — before this, findings-mode reviewers judged blind (run bc868de1) because only the builder's `previous=` carried the plan. A flag, not a new chain file — the chain past planning is identical, so a sibling file would only drift. `--from-adw-id` is validated as an opaque runner-minted session id (8 hex chars, `utils.new_id(8)`'s shape) and the resolved path is asserted beneath the sessions dir before any read — an id is never a path. #390: a findings rerun enters on the tree the failed run left behind (nothing commits on red), so its commit carries that whole change plus the fix-up while the builder writes `commit_message` for the findings alone — five reruns, five messages describing only the last edit, the worst a 7,937-line retirement filed as "Remove stale retired UI claims". The carried change is measured BEFORE the builder runs (afterwards the two are one diff) and named twice: `CARRIED_PROMPT` asks the builder for a subject covering the whole commit, and `CARRIED_BODY` states what the commit holds as a body paragraph appended in code, so the durable record is honest whatever comes back. Body text, never a trailer — `commit_all`'s one derived `Co-Authored-By` stays the last paragraph and the only one. A first-pass run, and a rerun whose prior change was already committed, measure nothing and keep an untouched message. |
...
| `scripts/sssf-gate.test.sh` | MOS-authored (#336/#343) | FAC-004/FAC-005 unit layer: runs the REAL `quality.py` + `adw_simple_sdlc.py` under stub siblings — cheap-first ordering, conditional pgTAP, verbatim failure tails, 3-round exhaustion exits with ZERO commits, green run commits exactly once. #343: trailer derivation on the real `git_helper.py` (per-substrate mapping, honest fallback, one trailer line, real scratch-repo commits incl. bogus builder-written trailers stripped; proven can-fail by dropping a mapping row), every committing chain passes its builder's model, and findings-rerun mode (no plan phase in the trace — the normal run is the can-fail control — plan reuse, session link, unchanged round caps, missing-prior refusal, traversal/absolute/non-hex `--from-adw-id` refused against a plan envelope planted outside the sessions dir). #367: the reviewer call carries the plan — `plan.md` is asserted present in the CURRENT session's context_handoff at review time and on the builder's `previous=` artifacts; proven can-fail by stubbing the adoption back out (the reviewer goes blind again), plus adoption-bound refusals with the hostile artifact riding SECOND behind a valid one (outside its session dir, file missing) asserting the handoff is EMPTY after refusal — red on one-pass validate-while-copying code — and a multi-artifact happy path.. #390: a findings rerun over a carried change far larger than its findings (63 files, +112 -7937) against a fix-up-shaped `commit_message` — the committed message names the carried change, the builder's own subject survives, the chain adds no trailer, the build prompt carries the same measurement, and the request record logs it; controls in the same pass: a first-pass run on the same dirty tree and a rerun on a clean tree both commit the builder's words untouched; proven can-fail by a perturbed copy with the body append dropped, which lands the whole change under the fix-up message again. |
...
| `scripts/sssf-quality-timeout.test.sh` | MOS-authored (#389) | Timed-out-check self-test: drives the REAL `quality._run` against real subprocesses that outlive a 1s budget — no exception, a failed result at exit 124, partial stdout AND stderr in `output_tail`, the budget named in both the tail and the console line, and an undecodable byte kept byte-exact in the log while the travelling tail stays encodable. The opposite direction runs in the same pass: a normally-failing check keeps its own exit code, verbatim tail, and unchanged console line. Three can-fail controls, each red on its own edge: drop the decode (the reported `TypeError` returns and no result reaches the builder), drop the artifact's error handler (the log write raises), drop `_envelope_safe` (the tail an agent receives cannot be serialized). |
...
| `scripts/sssf-provider-error.test.sh` | MOS-authored (#386) | Provider-fault vs model-fault self-test: the REAL `agent_pi.run` against a stub `pi` replaying the recorded 429 event sequence verbatim (empty content, `stopReason: "error"`, three auto-retries) records the upstream text and no content; the REAL `agents._parse_with_retries` then raises `ProviderFailure` naming the status — not the parse error — and burns ZERO correction sends. Both controls run in the same pass: a parsed-but-schema-invalid response, and an empty response with no provider error, each still raise the parse error. Proven can-fail by stripping the guard. |
...
| `.github/workflows/guards.yml` (sssf lines) | MOS-authored | Registers every sssf self-test in the guard lane; any PR touching `adws/**`, the stanza, `.env.sample`, or `justfile` re-proves conformance. |
