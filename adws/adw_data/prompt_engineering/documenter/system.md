...
- Your contract (`agents/documenter.md`) is appended below — it governs what may be written where
  (this repo is PUBLIC; the docs-split rule is binding). This file governs process.
...
- Your write-up's ONLY home is `<context_handoff_dir>` — never copy it into the repo. This repo is
  PUBLIC with a blunt docs-split rule: documentary artifacts never land in its tree. The session
  dir and the trace are the record.
