...
describe('issue 270 — the registry describes the pages that really render the frame', () => {
  ...
  it('the Money door reaches none of the shared follow-up queue — #428’s marker', () => {
    // A known-divergence marker, not a behaviour guard. It is here because the divergence kept
    // being rediscovered: comments across the codebase asserted it away and each reader believed
    ...
    // and resolver instead — and which edges count as runtime edges — is stated once, in
    // value-import-closure.ts. Read that before changing anything here.
    //
    ...
    // data-table import left behind stays green with the divergence entirely gone. It pins
    ...
    // — the same silent-drift failure it exists to prevent, one level up. The anchors have the
    // matching hazard: they are a reading list printed inside the red, so a renamed one would send
    // its reader to a file that no longer exists. Both lists are checked against disk first.
    const missing = [
      ...SHARED_FOLLOW_UP_QUEUE,
      ...COMMENTS_THIS_CASE_ANCHORS.map((anchor) => anchor.split(' — ')[0]),
    ].filter((mod) => !existsSync(join(SRC, mod)))
    expect(
      missing,
    ...
    expect(
      ...
      `THE #428 MARKER HAS GONE BLIND — it could not read part of the module graph below ` +
        ...
        `(add the mapping to tsconfig.app.json's \`paths\`), or — if the specifier does not ` +
        `resolve because it is a bundler asset the compiler never places — add its extension to ` +
        `BUNDLER_ASSET in shell/value-import-closure.ts. A module that DID resolve but is not ` +
        `TypeScript needs the walk taught to read it, not an exemption: BUNDLER_ASSET is only ` +
        `consulted on unresolved specifiers. Never silence this by narrowing what it looks at.`,
    ).toEqual([])
