 ...
 * non-translate `transform`, a malformed number — THROWS rather than returning a signature it
 * cannot stand behind. A guard that quietly certifies what it does not understand is the exact
 * defect this file exists to close, and a throw inside the guard is a red test, which is the
 * correct outcome: extend the canonicaliser, do not let it shrug.
 *
 ...
 * signs differently here) and drawing ATTRIBUTES (stroke-width, fill, opacity — geometry only,
 * which is the conservative side: it makes MORE things collide, never fewer).
 */
 ...
 * the running point, `H`/`V` become `L`, and `S`/`T` become `C`/`Q` with the reflected control
 * point made explicit — so `M3 9h18` and `M3 9L21 9` end up the same list.
 */
export function absolutise(segs: readonly Seg[]): Seg[] {
  const out: Seg[] = []
  ...
  for (const { cmd, args } of segs) {
    ...
    switch (up) {
      ...
      case 'S': {
        const [x1, y1] = lastCubicCtrl ? [2 * x - lastCubicCtrl[0], 2 * y - lastCubicCtrl[1]] : [x, y]
      ...
      case 'T': {
        // Snapshotted before use: `lastQuadCtrl` is re-derived from x1/y1 three lines down, and
        // reading it directly here makes the inference circular (TS7022).
        const reflectFrom: [number, number] | null = lastQuadCtrl
        const x1: number = reflectFrom ? 2 * x - reflectFrom[0] : x
        const y1: number = reflectFrom ? 2 * y - reflectFrom[1] : y
        x = args[0] + ox; y = args[1] + oy
        out.push({ cmd: 'Q', args: [x1, y1, x, y] })
        lastQuadCtrl = [x1, y1]; lastCubicCtrl = null
        break
...
function elementToPathData(el: Element): string {
  switch (el.localName) {
    ...
    default:
      return fail(`no geometry conversion for <${el.localName}>`)
  }
}
  ...
  for (const { cmd, args } of segs) {
    ...
    switch (cmd) {
      ...
      case 'A': {
        ...
        current.push(`A${q(rx / vb.w)},${q(ry / vb.h)} ${q(rot)} ${large} ${sweep} ${fx(ex)},${fy(ey)}`)
        break
      }
      case 'Z':
        current.push('Z')
...
function collect(el: Element, tx: number, ty: number, vb: ViewBox, into: string[]): void {
  for (const child of Array.from(el.children)) {
    ...
    if (!SHAPES.has(tag)) {
      fail(`<${tag}> is not a shape this guard can compare — extend glyph-shape.ts rather than trusting it`)
    }
