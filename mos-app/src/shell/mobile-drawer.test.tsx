...
describe('AC-021: More menu lists every authorized non-primary destination (admin)', () => {
  ...
  it('an org-wide admin sees Signals, Admin Settings, Profile — and the module links their routes admit', async () => {
    ...
    // Issue 781 (OD-WAY-95): Café is the module root — the capture list itself (the Log). Neither
    // Opening nor Log is a rail child (the module root IS the Log, and the opening is a door
    // row inside it). Admin holds Review + Pushes standing, so their children render.
    expect(screen.getByRole('link', { name: /^Café$/ })).toHaveAttribute('href', '/cafe')
    expect(screen.queryByRole('link', { name: /^Log$/ })).toBeNull()
    expect(screen.queryByRole('link', { name: /^Opening$/ })).toBeNull()
    expect(screen.getByRole('link', { name: /^Review$/ })).toHaveAttribute('href', '/cafe/review')
    expect(screen.getByRole('link', { name: /^Pushes$/ })).toHaveAttribute('href', '/cafe/pushes')
  ...
  it('Issue 781: a plain member (no affiliation, no lead role) sees Café root only — no children at all', () => {
    // A plain member with no Café affiliation and no lead role has no standing to write to
    // any Café surface; the More drawer therefore offers the module root alone. Kartika (an
    // affiliated kitchen member) sees Plan · Stock — see the destinations test for that.
