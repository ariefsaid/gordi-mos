 ...
 * the page `.record-split` grid, so it may coexist; AssistantPanel reads that same session through
 * `OverlayCompanionSlot` and contracts into the record-safe compact desktop regime
 * (OD-REDESIGN-80). Only shell-owner overlays collide and therefore remain mutually exclusive
 * here. (#190 carried a correction note while the panel was not yet on the shared host; the Deputy
 * chrome cutover landed the panel's half of the behaviour, so the v4 rationale is true again.)
 *
