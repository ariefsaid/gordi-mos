...
export const DESTINATIONS: Destination[] = [
  ...
  },
  {
    ...
    // KPI hub, OD-DASH-2 — replaces /sales) is its first content (finance/admin/manager-gated —
    // ADR-0050 D8, AC-128: manager is a financial VIEW-only tier); budget/COGS workbenches are
    // a documented future link (not rendered, and stay finance/admin only — no planning for
    // manager, FR-112). anyOf hides the whole destination for a role with no Plan children
    // (no dead-end — FR-410).
    anyOf: ['finance', 'admin', 'manager'],
    links: [
