...
// #404 port note: v4 sampled PlanQtyStepper (phone) + PlanQtyCell (desktop); dev retired both
// for the ONE typed control (DD-5) — PlanQtyField, dense on desktop. Commit state moved out of
// the field to the page-level kp-cell-status idiom (kitchen-plan-page.tsx), sampled here as-is.
import { PlanQtyField } from '@/components/kitchen/plan-qty-field'
import '@/pages/kitchen-plan-page.css'
...
export const v3Matrix = {
  ...
  canonicalImports: [
    ...
    { symbol: "PlanQtyField", file: "mos-app/src/components/kitchen/plan-qty-field.tsx", importPath: "@/components/kitchen/plan-qty-field" },
  ],
        ...
        <div className="v3-story-stack">
          <div className="v3-story-row">
            ...
            <div className="kp-cell-qty">
              <PlanQtyField itemName="Kopi Susu Gula Aren" qty={8} onSave={() => undefined} disabled={false} />
              <span className="kp-cell-status" role="status" aria-live="polite">Saving…</span>
            </div>
            <div className="kp-cell-qty">
              <PlanQtyField itemName="Cold Brew" qty={12} onSave={() => undefined} disabled={false} />
              <span className="kp-cell-status" role="status" aria-live="polite"><span className="kp-cell-tick" aria-hidden="true">✓</span> Saved</span>
            </div>
          </div>
          <div className="v3-story-row">
            <span className="v3-story-label">Desktop planning</span>
            <div className="kp-cell-qty">
              <PlanQtyField itemName="Kopi Susu Gula Aren" qty={8} onSave={() => undefined} disabled={false} dense />
              <span className="kp-cell-status" role="status" aria-live="polite">Saving…</span>
            </div>
            <div className="kp-cell-qty">
              <PlanQtyField itemName="Cold Brew" qty={12} onSave={() => undefined} disabled={false} dense />
              <span className="kp-cell-status" role="status" aria-live="polite"><span className="kp-cell-tick" aria-hidden="true">✓</span> Saved</span>
            </div>
          </div>
        </div>
