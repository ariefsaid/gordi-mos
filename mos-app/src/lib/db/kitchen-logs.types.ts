...
// ── Stock view (S4) — one display row per item the stream lists or holds ──────
// The read-only Stock view (FR-060/061) lists the stream's items with their two
...
export interface KitchenStockRow {
  wip_item_id: string
  /** false when the item is not on the stream's list (#222) and shows only for its balance. */
  on_stream?: boolean
