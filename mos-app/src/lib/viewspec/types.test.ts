...
describe('ENTITY_WHITELIST — AC-UV-001', () => {
  it('enumerates exactly the 8 MOS entities (T11/§3.1: business_units added for the deputy)', () => {
    expect(Object.keys(ENTITY_WHITELIST).sort()).toEqual([
      'business_units', 'objectives', 'people', 'sales_daily_revenue', 'sales_margin_daily',
  ...
  it('each entity carries schema + table (AC-UV-001)', () => {
    ...
    expect(ENTITY_WHITELIST.business_units).toMatchObject({ schema: 'shared', table: 'business_units' })
  })
