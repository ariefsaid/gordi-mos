...
describe('AC-HS13: drills — revenue/margin → /sales, ops-KPI → /ops, cascade → /work/cascade', () => {
  it('owner-cockpit money tiles link to /sales, ops-KPI to /ops, cascade to /work/cascade', async () => {
    mockListRevenue.mockResolvedValue([
    ...
    // Revenue + margin tiles drill to /sales
    const revenueTile = screen.getByRole('group', { name: /revenue/i })
    expect(revenueTile.closest('a')!.getAttribute('href')).toBe('/sales')
    const marginTile = screen.getByRole('group', { name: /gross margin/i })
    expect(marginTile.closest('a')!.getAttribute('href')).toBe('/sales')
