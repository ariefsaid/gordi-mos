...
const managerState: AuthState = {
  status: 'authenticated',
  viewer: { person: { ...mockPerson, id: 'manager-id' }, roles: [mockRole], isManager: true, accessRoles: [], affiliated: [], hasEmail: true },
  signOut: async () => {},
}
...
describe('AC-073 — read-only mode for non-editors', () => {
  it('hides status changer, PIC reassignment, checklist edit, and archive for unrelated viewer', async () => {
    ...
    const nonEditorAuth: AuthState = {
      status: 'authenticated',
      viewer: { person: mockPerson, roles: [mockRole], isManager: false, accessRoles: [], affiliated: [], hasEmail: true }, // mockPerson.id = VIEWER_ID, not in R/A
      signOut: async () => {},
    }
    mockGetTask.mockResolvedValue({ task, checklist: [], events: [] })
