...
describe('catalog row layout stays readable at phone width', () => {
  ...
  it('collapses a desktop split queue to identity + owner + progress', () => {
    ...
    // The two kept columns are fixed px (never stretch); Name caps at 640px with a trailing
    // gutter track absorbing space past it, same as the six-column grid.
    expect(split).toMatch(/grid-template-columns:\s*minmax\(0, 640px\)\s+92px\s+110px\s+minmax\(0, 1fr\)/)
