...
describe('Task 13 — TasksWorkspace canonical home (AC-116)', () => {
  ...
  // AC-V3-008c / AC-V3-008d — the dirty-record × Escape-key half of the dirty-leave contract
  // (brief step 4: "edit → Escape → confirm → Cancel keeps panel + draft; Discard leaves").
  // AC-V3-008 proved the dirty veto via the Close (✕) button; AC-V3-008b proved Escape closes a
  // CLEAN record with no dialog. These prove the SAME guard fires when the leave intent arrives
  // via the keyboard Escape path the host owns (I2) on a DIRTY record — split into two
  // single-cycle tests (Cancel-keeps / Discard-leaves) so each outcome is proved in isolation,
  // avoiding the cross-cycle async fragility of one guard Promise resolving between two attempts
  // (the host clears pendingLeave synchronously but its in-flight ref only in a microtask).
  //
  // Setup choice (resolved, not a guess): the dirty draft is an UNCOMMITTED edit
  // (fireEvent.change, no Enter/blur commit). The earlier failed-commit setup was unsound HERE —
  // on commit failure TaskSurface.handleUpdateField reverts localTask, and RecordField's
  // spec.value reset-effect then wipes the draft back to the saved value, so no draft ever
  // survives to be "retained". An uncommitted change is the clean representation of "a dirty
  // draft exists" and lets the Cancel branch genuinely prove the draft is intact. The field-local
  ...
  // (I5/NFR-V3-001).
  //
  // UNRESOLVED — flagged, not fixed (interaction-grammar owner disposition; do NOT guess): a
  // FIELD-FOCUSED dirty Escape is a different journey and is deliberately NOT asserted here.
  // RecordPanelHost attaches its Escape listener as a NATIVE addEventListener on the panel
  // <aside>, which fires during the bubble phase at the <aside> — BEFORE React's synthetic field
  // onKeyDown dispatches from the root container one level up. So Escape on a focused dirty field
  // today BOTH opens the retain/discard dialog AND reverts the field draft in one keystroke
  // ("Cancel" keeps the panel but not the typing). Whether field-Esc should discard-first per I5
  // (no dialog) or the guard should win and retain the draft pending the user's choice is an
  // I5-vs-I2 disposition for the owner. See the dirty-guard lane report.
  it('AC-V3-008c: a dirty record vetoed on Escape asks to discard; Cancel keeps the panel and the draft', async () => {
    ...
    fireEvent.click(document.querySelector('tr.task-row') as HTMLElement)
    const due = await screen.findByLabelText('Due') as HTMLInputElement
    // An uncommitted edit (no Enter/blur commit) → RecordField reports dirty and the tenant
    // attaches the leave-guard. The draft genuinely persists (no commit → no optimistic revert).
    fireEvent.change(due, { target: { value: '2026-08-01' } })
  ...
  it('AC-V3-008d: a dirty record vetoed on Escape leaves on Discard', async () => {
    const task = makeTask({ id: 'task-dirty-leave', title: 'Dirty leave task' })
    mockListTasks.mockResolvedValue([task])
    mockGetTask.mockResolvedValue({ task, checklist: [], events: [] })
    renderTable()

    await waitFor(() => screen.getByText('Dirty leave task'))
    fireEvent.click(document.querySelector('tr.task-row') as HTMLElement)
    const due = await screen.findByLabelText('Due') as HTMLInputElement
    ...
    // Discard: the close proceeds and the dirty draft is dropped — the panel unmounts.
