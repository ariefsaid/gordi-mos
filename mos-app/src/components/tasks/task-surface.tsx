...
function ViewSurface({
  ...
  // #756 AC-036: the viewer's own membership teams — the Team picker's option set. Never used
  // to widen the writer's reach (the DB's team-write RLS is the authority); this is the pre-
  // filtered "which teams may I re-home this task to" set.
  const [viewerTeamsDir, setViewerTeamsDir] = useState<ViewerTeamOption[]>([])
  ...
  const load = useCallback(() => {
    ...
    // #756 AC-036: the viewer's own teams for the Team picker. Non-blocking — a slow read
    // simply keeps the picker read-only ("Team not assigned yet") until the list resolves,
    // never blocks the record on skeleton. An unauthenticated viewer flows through with ''
    // and the list stays empty.
    if (viewerId) {
      listAuthorTeams(viewerId).then((rows) => {
        if (!isCurrent()) return
        setViewerTeamsDir(rows.map((row) => ({
          id: row.id, label: row.name, businessUnitId: row.business_unit_id,
        })))
      }).catch(() => {})
    }
  }, [taskId, viewerId])
  ...
  async function handleUpdateField(field: TaskViewerFieldKey, value: string | null) {
    ...
    switch (field) {
      ...
      // #756 AC-036: the record's Team picker writes team_id; the DB's mos._guard_tasks
      // enforces same-org + BU-equal — a client-side pre-filter (the picker's own scoping to
      // the viewer's teams filtered by BU) is UX, not authority.
      case 'team': patch.team_id = v; optimistic.team_id = v; break
    }
  ...
  const taskViewerAdapter = useMemo<RecordViewerAdapter | null>(() => {
    ...
    // #756 AC-036: only teams that share this task's BU are legal writes (mos._guard_tasks
    // enforces team.BU_id = task.BU_id). Filtering at the picker stage saves the writer a
    // foreseeable rejection round; the DB stays the authority.
    const eligibleViewerTeams = viewerTeamsDir.filter((t) => t.businessUnitId === localTask.business_unit_id)
    // #756 AC-039: Supervisor "inherited from …" resolves against the parent's A. If the
    // task has no work_line_id, or the work line loaded but has no accountable_person_id,
    // there is nothing to inherit — a `null` here means "no hint", not "unknown yet".
    const parentAccountable = localTask.work_line_id
      ? workLinesDir.find((wl) => wl.id === localTask.work_line_id)?.accountable_person_id ?? null
      : null
    const base = createTaskRecordAdapter({
      ...
      labels: {
        ...
        buPrefix: (name: string) => t('tasks.field.buPrefix', { name }),
        supervisorInherited: (name: string) => t('tasks.field.supervisorInherited', { name }),
        createdBy: t('tasks.field.createdBy'),
        createdByLine: (name: string, date: string) => t('tasks.field.createdByLine', { name, date }),
        adHoc: t('tasks.field.adHoc'),
      },
      recordLabels: {
