// PlanQtyField — the phone plan-qty control (DESIGN.md "Compact capture row").
//
...
//
// I5 inline-edit (OD-REDESIGN-22 / docs/interaction-contract.md, item 13): routed through the
// one primitive (useInlineCommit), same as its desktop sibling PlanQtyCell — Enter/Tab/blur
// COMMIT, Escape DISCARDS and restores the saved qty, an unchanged blur is a no-op (no
// needless upsert). Commit state (Saving… / ✓ Saved) is NOT rendered here: it belongs on the
// compact row's muted meta line, which renders only when it has something to say — inline it
// would reflow the row's one control mid-entry.
