...
describe('WipItemStepper — DD-18: the variance-note field survives being filled in', () => {
  it('DD-18(a): a floor worker can write a whole variance note — the field stays mounted and keeps focus across every keystroke', async () => {
    ...
    // The gate asks for an explanation, so they start writing it. B13: the visible label reads
    // "Note"; the accessible name keeps the item, so two open notes are told apart.
  ...
  it('DD-18(b): typing an off-plan quantity keeps the required note field reachable before blur', () => {
    ...
    // The visible label stays the short "Note"; the item lives in the accessible name only.
    expect(screen.getByText('Note', { selector: 'label' })).toBeInTheDocument()
  })
})
