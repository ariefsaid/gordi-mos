...
// (SIGNAL_CATEGORIES), so every surface that RENDERS a category — the toolbar filter, the row's
// meta line, the archive Feed picker — routes through this helper. The two locales can then never
// drift the way `attentionLabel` was written to prevent for attention words.
