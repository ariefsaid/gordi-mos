...
// When they sign in,
// Then they see the blocked screen — which names a role, never a person — with no app
// navigation, and sign-out returns them to /login (FR-016, OD-P1-10).
...
test('AC-003: orphan blocked screen', async ({ page }) => {
  ...
  // Goal-oracle: orphan screen — match the specific body text, not the footer.
  await expect(page.getByText(/your account isn't set up yet/i)).toBeVisible({ timeout: 10_000 })
  await expect(page.getByText(/contact your admin to get set up/i)).toBeVisible()
