...
export default async function globalSetup() {
  ...
  // ── 3b. ADMIN (dedicated e2e user + dedicated e2e person, granted admin) — AC-020 cascade catalog ─
  // Same dedicated-e2e pattern as RECOVERY: an e2e-namespaced person + the `admin` access role, so the
  // grant never mutates a shared dev persona. e2e logs in as this for the admin-gated /objectives page.
  await execSql(
    SUPABASE_URL,
    SERVICE_ROLE_KEY,
    `INSERT INTO shared.people (id, org_id, full_name, email)
  ...
  await execSql(SUPABASE_URL, SERVICE_ROLE_KEY, `
    -- Step 6 (ADR-0051): pending rows FK-reference materialized tasks, and tasks FK-reference
    -- process_runs — clear pending → tasks → runs, in that order, or the task wipe 23503s.
    DELETE FROM mos.process_run_pending_tasks WHERE org_id = '${orgId}';
    ...
    DELETE FROM mos.process_runs         WHERE org_id = '${orgId}';
  `)
