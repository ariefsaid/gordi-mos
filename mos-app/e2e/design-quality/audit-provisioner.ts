...
export function assertAuditOwnedRecord(definition: AuditFixtureRecordDefinition, namespace: string): void {
  ...
  assertIdentifier(definition.primaryKey ?? 'id', 'record primary key')
...
function validationErrors(
  ...
  for (const group of created) {
    ...
    for (const id of group.ids) {
      ...
      else if (seenIds.has(`${group.table}:${id}`)) errors.push(`fixture receipt duplicates owned ID ${group.table}:${id}`)
  ...
  if (phase === 'cleaned') {
    for (const [table, count] of createdCounts) {
      ...
      if (!cleanupEntry) errors.push(`fixture receipt has no cleanup result for ${table}`)
        ...
        if (cleanupEntry.deleted + cleanupEntry.remaining !== count) errors.push(`fixture receipt cleanup count is inconsistent for ${table}`)
  ...
  if (phase === 'provisioned' && remainingAuth.some((id) => !authIds.includes(id))) {
  ...
  if (seenIds.size > 0 || authIds.length > 0) {
  ...
  if (!isRecord(receipt.cleanupOnFailure)
    || typeof receipt.cleanupOnFailure.attempted !== 'boolean'
    || typeof receipt.cleanupOnFailure.completed !== 'boolean') {
    errors.push('fixture receipt is missing cleanup-on-failure status')
  } else if (phase === 'cleaned' && receipt.cleanupOnFailure.completed !== true) {
  ...
  if (!Array.isArray(receipt.ownedDatabaseIds)) {
    ...
    for (const group of receipt.ownedDatabaseIds) {
      ...
      for (const id of group.ids) {
        ...
        else if (declared.has(`${group.table}:${id}`)) errors.push(`fixture receipt duplicates owned database ID ${group.table}:${id}`)
...
export class AuditProvisioner {
  ...
  private readonly ownedAuthUserIds: string[] = []
  ...
  private async receipt(cleanup: AuditFixtureCleanupRow[], onFailure: { attempted: boolean; completed: boolean; error?: string }): Promise<AuditFixtureReceipt> {
    ...
    const allAuthUserIds = this.auth?.listUsers ? await this.auth.listUsers() : []
    const remainingAuthUserIds = allAuthUserIds.filter((id) => this.ownedAuthUserIds.includes(id))
    const receipt: AuditFixtureReceipt = {
      ...
      ownedDatabaseIds,
      ownedAuthUserIds: [...this.ownedAuthUserIds],
      remainingAuthUserIds,
  ...
  async provision(): Promise<AuditFixtureReceipt> {
    ...
    try {
      ...
      for (const identity of this.definitions.identities ?? []) {
        ...
        this.ownedAuthUserIds.push(userId)
      ...
      for (const [order, definition] of recordOrder(this.definitions.records ?? []).entries()) {
        ...
        this.createdRecords.push({ table: definition.table, id: owned.id, primaryKey: definition.primaryKey ?? 'id', order })
        await this.emit(await this.receipt([], { attempted: false, completed: false }))
      }
  ...
  async cleanup(options: { onFailure?: boolean } = {}): Promise<AuditFixtureReceipt> {
    ...
    try {
      for (const group of orderedGroups) {
        ...
        cleanup.push({ table: group.table, deleted: Math.max(group.ids.length - remaining, 0), remaining })
      }
      let existingAuthUserIds = new Set<string>()
      if (this.ownedAuthUserIds.length > 0 && !this.auth) {
        throw new Error('audit fixture auth cleanup requires an auth admin client')
      }
      if (this.ownedAuthUserIds.length > 0 && !this.auth?.listUsers) {
        throw new Error('audit fixture auth cleanup requires user listing for cleanup verification')
      }
      if (this.ownedAuthUserIds.length > 0) {
        existingAuthUserIds = new Set(await this.auth!.listUsers!())
      }
      for (const id of [...this.ownedAuthUserIds].reverse()) {
        if (!this.auth || !existingAuthUserIds.has(id)) continue
  ...
  seedReceipt(receipt: AuditFixtureReceipt): void {
    ...
    if (!Array.isArray(receipt.created) || !Array.isArray(receipt.ownedAuthUserIds)) {
    ...
    for (const [order, group] of receipt.created.entries()) {
      const primaryKey = receipt.ownedDatabaseIds?.find((owned) => owned.table === group.table)?.primaryKey ?? 'id'
      for (const id of group.ids) this.createdRecords.push({ table: group.table, id, primaryKey, order })
    }
    this.ownedAuthUserIds.length = 0
    this.ownedAuthUserIds.push(...receipt.ownedAuthUserIds)
    this.beforeSentinels.length = 0
...
export async function cleanupAuditFixtureReceipt(
  ...
  const validation = validateAuditFixtureProvisionedReceipt(receipt, {
    candidateSha: options.candidateSha,
    sessionId: options.sessionId,
  })
  if (!validation.ok) throw new Error(`invalid audit fixture receipt for cleanup: ${validation.errors.join('; ')}`)
  const provisioner = new AuditProvisioner({ ...options, definitions: {
    records: receipt.created.flatMap((group) => group.ids.map((id) => ({
      table: group.table,
      id,
      namespace: receipt.namespace,
      columns: { id, namespace: receipt.namespace },
    }))),
...
export function createLocalAuditSqlClient(url: string, serviceKey: string): AuditFixtureSqlClient {
  ...
  if (!['localhost', '127.0.0.1', '::1'].includes(host)) throw new Error('audit fixtures require a local database')
  const request = async (query: string): Promise<unknown> => {
    ...
    if (!response.ok) throw new Error(`audit fixture SQL failed (${response.status})`)
    try { return await response.json() }
    catch { throw new Error('audit fixture SQL returned invalid JSON') }
  }
  return { execute: request, query: async (query) => rows(await request(query)) }
}
...
export function createLocalAuditAuthClient(url: string, serviceKey: string): AuditFixtureAuthClient {
  ...
  return {
    createUser: async (input) => {
      ...
      return { data: isRecord(body) ? { user: { id: typeof body.id === 'string' ? body.id : undefined } } : undefined }
    },
    deleteUser: async (id) => {
    ...
    listUsers: async () => {
      const { response, body } = await request('/auth/v1/admin/users?per_page=1000')
      ...
      return body.users.flatMap((user) => isRecord(user) && typeof user.id === 'string' ? [user.id] : [])
    },
  }
}
