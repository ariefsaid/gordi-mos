...
export async function localSqlRead<T = Record<string, unknown>>(
  ...
  if (!response.ok) throw new Error(`Local fixture SQL read failed (${response.status})`)
  const data: unknown = await response.json()
  if (!Array.isArray(data)) throw new Error('Local fixture SQL read returned a non-rowset')
  return data as T[]
}
