...
`mos-app/public/`). Deployed at `https://ops.gordi.id/mos`; staging at `gordi-mos.pages.dev/mos`.
Not a native app; a phone-first web app does not make its design language native.
...
Four confirmed personas (owner-ratified 2026-07-06, `docs/jtbd.md` v0.3):
...
- **Owner-director** (Arief) — whole-company scope. Job: *see the company money position
...
MOS is the **operating system for the whole company** — it replaces a dormant Notion "Management OS"
and, more importantly, the forked Google Sheets the company actually runs on today. It carries task
ownership + RACI, weekly and daily updates, per-Activity operations (plan / log / stock / review),
shared reference data, budgets and pricing, and money follow-ups (B2B AR + retail pending bills).
...
reduced model of it. Anything written before E6 describes an older bar
(`docs/requirements-evolution.md` is the timeline).
...
- **Systems it sits beside:** ESB (POS/ERP — source of sales, products, BOM; MOS pushes production
  ...
  future Gordi ops apps via schema seams, and the Google Sheets being retired.
...
- `docs/jtbd.md` v0.3 — owner-ratified persona × destination job stories; the product-intent oracle.
- `CONTEXT.md` — binding domain glossary.
- `docs/requirements-evolution.md` — the E1→E6 era timeline; earlier docs are history, stamped.
- `docs/decisions.md` — numbered owner decisions (OD-*); `docs/adr/` — architecture decisions.
...
<!-- Interview basis: repository evidence (docs/jtbd.md v0.3, CONTEXT.md,
docs/requirements-evolution.md, docs/decisions.md, mos-app/ routes, CLAUDE.md) plus a live owner
answer round on 2026-07-27 covering redesign scope and the incumbent-world decision. Visual world
decisions are deliberately absent from this file — they belong to new-work / DESIGN.md. -->
